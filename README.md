...
The Jinja2 engine, `rebar3 jinja`, `beamai_jinja_transform` and a shared layer
...
| Removed | Replacement |
|---|---|
| `beamai_mustache:bootstrap/0,1` | nothing -- templates are compiled by the build |
| `beamai_mustache:reload/0` | `beamai_mustache_dev:reload/1` (dev only) |
| `beamai_mustache:render(Name, Ctx)` where `Name` is a string | `view_name:render(Ctx)`, or `beamai_mustache:render(view_name, Ctx)` |
| `application:start(beamai_render)` | nothing -- beamai_render is a library application |
...
views/index.mustache
   |
   +- beamai_mustache_scanner     lexer: text and tag tokens, standalone lines, delimiters
   +- beamai_mustache_parser      recursive descent -> AST
   +- beamai_mustache_ast         merge text, drop empty bodies, resolve partials
   |
   +- beamai_mustache_compiler    AST -> Erlang abstract forms      (the only such implementation)
         |
         +- rebar3_beamai_render     erl_prettypr -> _gen/view_index.erl -> .beam
...
render_stack(S, I) ->
    [I, <<"<h1>">>,
     beamai_mustache_rt:escape(beamai_mustache_rt:lookup([header], S)),
     <<"</h1>\n">>,
     sec_1(S, I)].

sec_1(S, I) ->
    case beamai_mustache_rt:lookup([items], S) of
        []                       -> [];
        ...
        F when is_function(F, 1) -> beamai_mustache_rt:section(F(hd(S)), fun sec_1_body/2, S, I);
        V                        -> beamai_mustache_rt:section(V, fun sec_1_body/2, S, I)
    end.
...
time, and the `beamai_render` dependency, whose `beamai_mustache_rt` the generated
...
{deps, [{beamai_render, {git, "https://github.com/DavidAlphaFox/beamai_render.git",
                  {tag, "v0.4.0"}}}]}.

{plugins, [{rebar3_beamai_render, {git_subdir, "https://github.com/DavidAlphaFox/beamai_render.git",
                            {tag, "v0.4.0"}, "rebar3_beamai_render"}}]}.

{provider_hooks, [{pre, [{compile, mustache}]}]}.
```

That is genuinely all of it -- there is no `mustache_opts` above because every
option has a working default. The plugin lives in a subdirectory of the beamai_render
...
DEP_PLUGINS = rebar3_beamai_render
dep_rebar3_beamai_render = git https://github.com/DavidAlphaFox/beamai_render.git v0.4.0
```
...
beamai_mustache:inline(~"Hello {{name}}!", #{name => Name}) -> binary().
```
...
```erlang
beamai_mustache_dev:check()          % one-shot environment self-test
beamai_mustache_dev:stale(view_index)
beamai_mustache_dev:reload(view_index)
beamai_mustache_dev:reload(all)
```
...
greet(Name) -> beamai_mustache:inline(~"Hello {{name}}!", #{name => Name}).
...
are therefore recognised as calls to `beamai_mustache:inline/2` whose first
...
It builds in a temporary copy, because beamai_render and rebar3_beamai_render have to reach
