...
Jinja2 引擎、`rebar3 jinja`、`beamai_jinja_transform`，以及两个引擎共用的底层。
...
| 移除 | 替代 |
|---|---|
| `beamai_mustache:bootstrap/0,1` | 无 —— 模板由构建期编译 |
| `beamai_mustache:reload/0` | `beamai_mustache_dev:reload/1`（仅开发期） |
| `beamai_mustache:render(Name, Ctx)`，`Name` 是字符串 | `view_name:render(Ctx)`，或 `beamai_mustache:render(view_name, Ctx)` |
| `application:start(beamai_render)` | 无 —— beamai_render 是 library application |
...
用到的部分全部在 `beamai_mustache_rt` 中重新实现，只依赖 OTP。
...
views/index.mustache
   |
   +- beamai_mustache_scanner     词法：文本与 tag token、standalone 行、分隔符
   +- beamai_mustache_parser      递归下降 -> AST
   +- beamai_mustache_ast         合并文本、消除空 body、解析 partial
   |
   +- beamai_mustache_compiler    AST -> Erlang abstract forms      （唯一的实现）
         |
...
需要**两个**条目：plugin 在构建期编译模板，`beamai_render` 依赖提供生成模块运行期调用的 `beamai_mustache_rt`。
...
真的就这些 —— 上面没有 `mustache_opts`，因为每个选项都有可用的默认值。plugin 在 beamai_render 仓库的子目录里，所以是 `git_subdir` 而不是 `git`。
...
```erlang
beamai_mustache_dev:check()          % 一次性环境自检
...
**没有 `~mustache` 这种 sigil。** Erlang 的 sigil 是封闭集合，自定义的过不了词法。所以内联模板靠"识别第一个参数是 binary 字面量的 `beamai_mustache:inline/2` 调用"实现。`~"..."` 是 OTP 27 的标准字符串 sigil，产出的正是这样的字面量。
...
它在临时副本里构建，因为 beamai_render 和 rebar3_beamai_render 必须通过 `_checkouts` 接进来（rebar3 没有 `path` resource），而把仓库根链进仓库内的目录会形成循环。脚本会打印生成的模块列表和渲染出的页面。
