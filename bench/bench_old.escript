#!/usr/bin/env escript
%%! -noshell
%% Benchmark harness for beamai_render v0.3.7 (commit 82c01eb), which needs ailib.
...
main([EbinDir, AilibDir, BenchDir, NStr, ItemsStr]) ->
    ...
    {ok, _} = beamai_mustache_loader:start_link(),
    Views = list_to_binary(filename:join([BenchDir, "old", "views"])),
    ok = beamai_mustache:bootstrap(#{views => Views}),
    Ctx = bench_ctx:context(Items),
    Out = beamai_mustache:render("page", Ctx),
    ok = file:write_file(filename:join(BenchDir, "out_old.txt"), Out),
    io:format("bytes ~p~n", [byte_size(Out)]),
    report("render/2        ", fun() -> beamai_mustache:render("page", Ctx) end, N).
