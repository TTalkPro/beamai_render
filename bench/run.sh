...
#
# The two versions cannot share a VM -- both define beamai_mustache -- so each runs
...
echo "==> extracting beamai_render $OLD_REV into $WORK/old"
...
echo "==> building beamai_render $OLD_REV"
erlc -o "$WORK/old/ebin" -pa "$AILIB" "$ROOT"/../beamai_render/bench/../src/*.erl 2>/dev/null || \
    erlc -o "$WORK/old/ebin" -pa "$AILIB" "$WORK"/old/src/*.erl

echo
echo "==> current (templates compiled to modules)"
escript bench/bench_new.escript _build/default/lib/beamai_render/ebin bench "$N" "$ITEMS"
echo
