...
| 模块 | 行数 | 职责 |
|---|---|---|
| `beamai_mustache.erl` | 31 | 门面：`render/2`、`bootstrap/0,1`、`reload/0` |
| `beamai_mustache_loader.erl` | 342 | gen_server，持有 ets 表 `beamai_mustache`，负责扫描目录、parse、缓存 |
| `beamai_mustache_parser.erl` | 368 | 词法+语法分析，产出 IR。改自 [bbmustache](https://github.com/soranoba/bbmustache)（MIT） |
| `beamai_mustache_runner.erl` | 107 | 栈式解释器，消费 IR 产出 binary |
| `beamai_render_app.erl` | 13 | application 回调 |
| `beamai_render_sup.erl` | 24 | supervisor，仅挂 `beamai_mustache_loader` |
...
views/**/*.mustache
   │
   ├─ beamai_mustache_loader:bootstrap/0,1     启动时全量扫描 recursive_dir/1
   │     └─ load/3 → beamai_mustache_parser:parse/1
   │           └─ ets:insert  {c,Name} => IR
   │                          {t,Name} => [PartialName]
   │
   └─ beamai_mustache:render/2
         ├─ erlang:get(Template)                    ← 进程字典二级缓存
         ├─ beamai_mustache_loader:template/1
         │     ├─ ets:lookup {t,Name}               ← 深拷贝
         │     ├─ template_partial/1 递归 fold + maps:merge
         │     └─ ets:lookup {c,Name}               ← 深拷贝
         ├─ erlang:put(Template, Code)
         └─ beamai_mustache_runner:render/2             ← 栈式解释，binary append
```
...
`beamai_mustache_loader.erl:38-51`。IR 树存在 ets 里，每次 `ets:lookup` 都会把整棵树从 ets 的独立内存区**拷贝**到调用进程堆上。模板越大、partial 越多，拷贝越贵。

> 这是去掉 ets 最强的技术论据：编译成 BEAM 模块后，静态文本与结构进入模块的 **literal pool**，跨进程按引用共享，零拷贝。

### 4.2 进程字典缓存架空了 ets

`beamai_mustache.erl:19-26`：

```erlang
Run = case erlang:get(Template) of
          undefined -> Code = beamai_mustache_loader:template(Template),
...
`beamai_mustache_loader.erl:98-105`：ets 表由 gen_server 创建且为 `protected`。loader 崩溃时：

- ets 表随之销毁（数据可 lazy load 恢复，尚可接受）
- **`bootstrap/1` 设置的 `view_path` / `suffix` 回退到 `init/1` 的默认值**（`CWD/views` + `.mustache`），行为静默改变

`beamai_render_sup.erl` 用 `intensity => 1, period => 5`，容错窗口极窄。

### 4.4 首载路径重复计算

`beamai_mustache_loader.erl:53-61` 的 `template_partial/1` 递归遍历所有 partial 并 `maps:merge`，每个模板首次渲染时都跑一遍，O(partials)。

### 4.5 模板名 `binary_to_atom`

`beamai_mustache_loader.erl:35`：
...
`beamai_mustache_runner.erl:105`：
...
`beamai_mustache_parser.erl:177`：
...
`keys/1`（`beamai_mustache_parser.erl:246-251`）对 `.` 产出 `['.']`，runner 走 `ai_maps:get(['.'], Ctx)` 去 map 里找键 `'.'`，永远取不到。标准 mustache 的隐式迭代器不可用。
...
`merge_continuous_binary/1` 与 `remove_empty_section/1` 都用 `Acc ++ [I]` 累加（`beamai_mustache_parser.erl:328,338,341,363,366`）。

### P2 · `merge_continuous_binary/1` 递归不完整

`beamai_mustache_parser.erl:336-342` 只对 `{section, ...}` 递归下钻，**`{has, ...}` 内部的 IR 不做合并**，导致 `{{+x}}` 块内的连续静态文本在运行期被反复 binary append。

### P3 · `remove_empty_section/1` 含死代码且只处理顶层

`beamai_mustache_parser.erl:355-368`：只遍历顶层不递归；其中 `[<<>>]` 分支永不成立 —— `?ADD` 宏保证空 binary 不会被插入 IR，且 IR 里是 `{binary, B}` 而非裸 binary。

### P4 · 运行期 binary append

`beamai_mustache_runner.erl` 全程 `<<Acc/binary, X/binary>>`。ERTS 对尾部追加有优化，但 `run_section/7` 和 lambda 分支会重新起 `<<>>` 累加器，优化链被打断。输出应改为 **iolist**。
