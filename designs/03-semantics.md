...
| `{{+x}}...{{/x}}` | has，纯布尔判定，**不压栈不迭代**（beamai_render 扩展） |
| `{{-x}}...{{/x}}` | inverted has，同上取反（beamai_render 扩展） |
| `{{*f}}` | lambda（beamai_render 扩展），见 §5 |
...
现有实现（`beamai_mustache_parser.erl:180`）只是把 Indent 作为普通文本插在 partial 前面，即只缩进第一行。这不符合 spec，需要在编译期把 Indent 作为参数传给 partial 模块：
...
| 项 | beamai_render 行为 | 原因 |
|---|---|---|
| `{{+x}}` / `{{-x}}` | 非标准的 has / inverted-has | beamai_render 扩展，spec 无此语法 |
| `{{*f}}` | 非标准的 lambda | beamai_render 扩展 |
| key 类型 | **atom**（spec 用 string） | 沿袭 v0.3.5 的决定，Erlang 侧 map 用 atom key 更自然 |
| lambda 返回值 | 不做二次模板解析 | spec 的 Lambdas 可选模块要求返回值再解析一遍；beamai_render 编译期方案下代价过高 |
