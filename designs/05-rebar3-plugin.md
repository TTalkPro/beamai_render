...
> **决策 D3：rebar3 为主构建；plugin 放本仓 `rebar3_beamai_render/` 子目录，通过 `{subdir, ...}` 引用。**
...
```erlang
{deps, [{beamai_render, {git, "https://github.com/DavidAlphaFox/beamai_render.git", {tag, "v0.4.0"}}}]}.

{plugins, [
    {rebar3_beamai_render, {git, "https://github.com/DavidAlphaFox/beamai_render.git",
                     {tag, "v0.4.0"}}, {subdir, "rebar3_beamai_render"}}
]}.
...
hash **由 beamai_render 侧的 `beamai_mustache_compiler:source_hash/2` 计算，plugin 只负责调用**，不得自行实现（[04 §4.1](04-codegen.md#41-source-hash-的计算归属)）。`-mustache_source` 的 `opts` 字段同时供 dev 热重载（[T27](../tasks/T27.md)）复现编译，二者共用同一份口径。
...
{profiles, [
    {test, [{deps, []},                       %% spec 用 OTP 自带的 json 模块解析
            ...
            {plugins, [{rebar3_beamai_render, {path, "rebar3_beamai_render"}}]}]}
]}.
...
测试 profile 用 `{path, "rebar3_beamai_render"}` 直接引本地 plugin 子目录，实现「同仓改动即时生效」—— 这正是 D3 选同仓的理由。
