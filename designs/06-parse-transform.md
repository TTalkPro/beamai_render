...
%% (b) 内联模板
greet(Name) ->
    beamai_mustache:inline(~"Hello {{name}}, you have {{count}} msgs.",
                       #{name => Name, count => 3}).
```

`beamai_mustache_transform:parse_transform/2` 单遍扫描 forms：收集 attribute → 校验 → 生成/替换 → 返回新 forms。
...
%% {{@ hello}}  →  my_i18n_rt:t(hello, beamai_mustache_rt:lookup([locale], S))
compile_tag($@, [Key], [], _Opts) ->
    L = 0,
    {call, L, {remote, L, {atom, L, my_i18n_rt}, {atom, L, t}},
     [{atom, L, Key},
      {call, L, {remote, L, {atom, L, beamai_mustache_rt}, {atom, L, lookup}},
...
| 机制 | 职责 |
|---|---|
| `-mustache_tag` + parse_transform | **声明与校验**：编译期检查目标模块确实 `-behaviour(beamai_mustache_ext)`、导出了 `markers/0` 与 `compile_tag/4`、且 `markers/0` 的返回值包含声明的字符，不匹配则报编译错误 |
...
parse_transform 匹配 `beamai_mustache:inline/2` 且**第一个参数是 binary 字面量**时，编译期把模板展开为原地 iolist 构造：
...
greet(Name) ->
    erlang:iolist_to_binary(
      begin
          S__1 = [#{name => Name}],
          [<<"Hello ">>,
           beamai_mustache_rt:escape(beamai_mustache_rt:lookup([name], S__1)),
           <<"!">>]
      end).
```
...
`beamai_mustache:inline/2` 本身有真实实现（运行期 scanner + parser + 解释求值）。所以：
...
`beamai_mustache:inline(Tpl, Ctx)` 中 `Tpl` 是变量时，transform 原样保留调用，走运行期路径。可选地在 `warnings_as_errors` 下发一条 warning 提示无法优化。
...
parse_transform(Forms, Opts) ->
    ...
    %% pass 2：展开 beamai_mustache:inline/2，剔除已消费的 attribute
...
| | (a) 扩展点 | (b) 内联 | (c) 编译入口 |
|---|---|---|---|
| 触发 | `-mustache_tag` | `beamai_mustache:inline/2` | `-mustache_template` |
