...
- [x] `test/beamai_mustache_test_lib.erl`：spec 加载（`json:decode/1`，零依赖）、binary key → atom 转换、用例名归一与去重、阶段判定
- [x] `test/beamai_mustache_spec_tests.erl`：spec 驱动，每个 case 一个具名测试；inventory 断言（12/14/42/22/12/34，总数 136）真跑
- [x] `test/beamai_mustache_test_lib_tests.erl`：支撑库自测，全程真跑
- [x] `test/beamai_mustache_ext_tests.erl`：beamai_render 扩展语法（`{{+}}` / `{{-}}` / `{{*}}`）、section 分派表、falsy 集合、escape 字符集的用例表
...
- [x] 从 `beamai_mustache_parser` 拆出 `beamai_mustache_scanner`（`split_tag/2`、`standalone/3`、delimiter 处理）
...
**目标**：`beamai_mustache_compiler` 可用，语义切换到标准 context stack。

- [x] `beamai_mustache_rt`：`lookup/2`（栈回溯）、`escape/1`（单遍扫描，修 [B3](01-current-state.md#b3-·-html-escape-缺分号且极慢（在-ailib-中）)）、`section/4`、`lambda/2`、`truthy/1`
- [x] `beamai_mustache_compiler:forms/2` + `compile_inline/3`
...
- [x] `beamai_mustache_ext` behaviour（[06 §2.1](06-parse-transform.md#21-behaviour)）
...
- [x] `beamai_mustache_dev`：mtime/hash 热重载（读 `-mustache_source` attribute，无额外状态）
- [x] `examples/` 按新语义重写
