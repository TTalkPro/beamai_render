...
beamai_render 现在是一个 mustache 引擎。要再加一套 Jinja2，问题不是「能不能写一个 Jinja parser」——
那是纯工作量；问题是**两套引擎如何共处**，才不会出现下面这三种结局：

1. 复制粘贴一份 `beamai_mustache_*` 改名成 `beamai_jinja_*`，从此每个 bug 修两遍；
...
| 模块 | 内容 | 来源 |
|---|---|---|
| `beamai_html_forms` | Erlang abstract forms 构造器：`a/2` `v/2` `bin/2` `mklist/2` `fn/4` `spec/4` `param/3` `uses_var/2` `remotes/2` `anno/1` | 从 `beamai_mustache_compiler` 私有函数抽出 |
| `beamai_html_text` | UTF-8 规范化：`template/1` `path/1` `opts/1` `source/1` `to_list/1` | 现 `beamai_mustache_text` 改名，保留 `beamai_mustache_text` 作转发壳 |
| `beamai_html_escape` | `escape/1`（单遍扫描）· `to_binary/1`（含 `float_to_binary(F,[short])`） | 从 `beamai_mustache_rt` 抽出 |
| `beamai_html_engine` | behaviour：`forms/2` · `source_hash/2` · `module_name/2` · `suffix/0` · `attribute/0` · `parse/2` | 新写 |
| `beamai_html_path` | 模板路径解析与 `.erl` 静态扫描 | 现 `beamai_mustache_path` 泛化（属性名参数化） |

`beamai_html_forms` 是这次抽取里最有价值的一块。它包含两个**已经踩过坑、且下一个引擎必然会再踩一次**的实现细节：
...
这两条写在 `beamai_mustache_compiler:bin/2` 的注释里。抽成共享模块后，Jinja 引擎自动继承，而不是等着被同一个坑咬第二次。
...
## 2. `beamai_html_engine` behaviour

plugin 与 dev 模块面向它编程，从此不认识具体引擎：

```erlang
-module(beamai_html_engine).
...
`beamai_mustache_compiler` 已经导出了 `forms/2`、`source_hash/2`、`normalize_opts/1`，
`beamai_mustache_ast` 已经导出 `module_name/2`。因此 mustache 侧的适配只需要一个薄壳
`beamai_mustache_engine`，不动核心模块的任何逻辑。

**为什么 `attribute()` 是 callback 而不是常量 `template_source`。**
把 mustache 的 `-mustache_source` 改名会让所有已构建产物一次性失效、
`beamai_mustache_dev` 与旧 `.erl` 全部对不上，收益却只是少一个 callback。
...
理由是 `rebar3_beamai_render_scan:partials_of/1` 与孤儿清理、反向依赖表都读这个函数；
...
beamai_render/
├── src/
│   ├── beamai_html_forms.erl          # 新 · 共享 forms 构造器
│   ├── beamai_html_text.erl           # 新 · 现 beamai_mustache_text 迁入
│   ├── beamai_html_escape.erl         # 新 · 现 beamai_mustache_rt 的 escape/to_binary 迁入
│   ├── beamai_html_engine.erl         # 新 · behaviour 定义
│   ├── beamai_html_path.erl           # 新 · 现 beamai_mustache_path 泛化
│   │
│   ├── beamai_mustache*.erl           # 既有 · 只做「改为调用共享层」的机械替换
│   ├── beamai_mustache_engine.erl     # 新 · behaviour 适配壳
│   │
│   ├── beamai_jinja.erl               # 门面
│   ├── beamai_jinja_scanner.erl       # 词法：三种定界符 + 空白控制 + raw
│   ├── beamai_jinja_lexer.erl         # 表达式词法
│   ├── beamai_jinja_expr.erl          # 表达式语法（优先级爬升）
│   ├── beamai_jinja_parser.erl        # 语句结构 → AST
│   ├── beamai_jinja_ast.erl           # AST 后处理 pass
│   ├── beamai_jinja_compiler.erl      # AST → forms          核心
│   ├── beamai_jinja_engine.erl        # behaviour 适配壳
│   ├── beamai_jinja_rt.erl            # 生成代码的运行期依赖
│   ├── beamai_jinja_filters.erl       # 内置 filter 库
│   ├── beamai_jinja_tests.erl         # 内置 test 库
│   ├── beamai_jinja_ext.erl           # 自定义 filter/test/tag 的 behaviour
│   ├── beamai_jinja_transform.erl     # parse_transform
│   └── beamai_jinja_dev.erl           # 开发期热重载
├── include/
│   ├── beamai_mustache.hrl            # 既有
│   ├── beamai_html.hrl                # 新 · 两引擎共用的类型（loc、path、error 外壳）
...
7. **`beamai_jinja_compiler` 是唯一的 Jinja AST → forms 实现。** plugin 与 parse_transform 都不得旁路它。
8. **生成模块只依赖 `beamai_jinja_rt`、`beamai_jinja_filters`、`beamai_jinja_tests`、其它生成模块，以及用户显式注册的 filter/test 模块。**
...
10. **共享层模块不得引用任何 `beamai_mustache_*` 或 `beamai_jinja_*` 模块。** 依赖方向单向向下，
...
| 风险 | 缓解 |
|---|---|
| `beamai_mustache_compiler` 抽走私有函数后行为漂移 | 抽取是**纯机械替换**：函数体逐字搬运，只改模块名。抽取提交里不得夹带任何逻辑改动，`rebar3 eunit && rebar3 ct` 必须在抽取前后产出相同结果 |
| `beamai_mustache_text` 改名破坏外部调用方 | 保留 `beamai_mustache_text` 作为转发壳并标 `-deprecated`，v0.6 再删 |
| 生成产物字节变化触发全量重编 | 抽取不改 `?BEAMAI_MUSTACHE_VSN`、不改 `normalize_opts/1`，因此 stamp 不变，已构建产物继续有效。**这一条要在验收里实测**：抽取前后对同一模板生成的 `.erl` 应逐字节相同 |
...
生成模块前缀不同即可避免命名冲突；`rebar3_beamai_render_check` 的重名检测本就在**全项目范围**内做，
