...
本文档是 `beamai_jinja_scanner` / `beamai_jinja_lexer` / `beamai_jinja_expr` / `beamai_jinja_parser` 四个模块的规格说明。
...
不能用 `binary:match/2`，必须由 `beamai_jinja_lexer` 边词法分析边推进（见 §3.4）。
...
| `{% trans %}` / i18n 扩展 | 属于 Jinja2 的扩展体系而非语言本体。beamai_render 已有 `{{@ }}` 风格的扩展点思路，i18n 应走 [12 §5](12-jinja-toolchain.md) 的自定义 tag |
