...
`beamai_jinja_rt:to_binary/1` 因此带一个小型的 Python `repr` 实现（约 60 行）。
...
运行期字符串下标由 `beamai_jinja_rt:subscript/2` 处理，**不做 `binary_to_atom/2`**——
...
名字解析 `beamai_jinja_rt:resolve(Name, Scope)`：从栈顶向外找第一个含该名字的 map frame，
...
```erlang
%% {% set a = 1 %}{{ a }}{% set a = 2 %}{{ a }}
V1 = beamai_jinja_rt:bind(V0, a, 1),
E1 = beamai_jinja_rt:resolve(a, V1),
V2 = beamai_jinja_rt:bind(V1, a, 2),
E2 = beamai_jinja_rt:resolve(a, V2),
[E1, E2]
```
...
| 概念 | 表示 | `{{ }}` 输出 | 属性访问 | 参与运算 |
|---|---|---|---|---|
| 未定义 | `undefined` | `<<>>` | `undefined`（可链式） | 抛 `{beamai_jinja, {undefined_operation, Op}}` |
...
`{escape, true}`（默认）时每个 `{{ e }}` 编译成 `beamai_jinja_rt:escape(E)`；
`{escape, false}` 时编译成 `beamai_jinja_rt:to_binary(E)`。
...
escape(V)         -> beamai_html_escape:escape(beamai_jinja_rt:to_binary(V)).
```
...
这张分类表必须写死在 `beamai_jinja_filters` 的每个函数上，且**每条都要有 fixture**。
搞错一条就是一个 XSS。
...
**不要复用 `beamai_mustache_rt:truthy/1`。** 两者语义不同，共用会让其中一个引擎悄悄错。
...
`beamai_jinja_rt:to_list/1` 会把可迭代物实体化成 list。这排除了惰性流，
...
filter 名在编译期已知，因此 `{{ x|upper }}` 编译成 `beamai_jinja_filters:upper(X)`——
...
因此这一组无法完全静态绑定，走 `beamai_jinja_rt:apply_named/3` 的白名单派发——
