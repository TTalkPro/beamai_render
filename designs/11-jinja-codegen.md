...
`beamai_jinja_compiler` 的规格。前置阅读：[04](04-codegen.md)（mustache 的对应文档，
...
| `{name, _, N}` | `beamai_jinja_rt:resolve(N, V)` |
| `{attr, _, E, K}` | `beamai_jinja_rt:attr(E', K)` |
| `{sub, _, E, I}` | `beamai_jinja_rt:subscript(E', I')` |
| `{binop, _, Op, A, B}` | `beamai_jinja_rt:Op(A', B')`，**不是** Erlang 原生运算符（见 §3.3） |
...
| `{test, _, T, E, Args, Neg}` | `beamai_jinja_tests:T(E', ArgsMap)`，取反时包 `not` |
| `{call, _, F, Args}` | 见 §3.4 |
...
| 内置函数（`range` `dict` `namespace`） | `beamai_jinja_rt:builtin(N, Pos, Kw)` |
| 其它（作用域里的 `fun`） | `beamai_jinja_rt:call(F', Pos, Kw)` |
...
{Res, V1} = case beamai_jinja_rt:truthy(<a>) of
                true  -> {[<X>], <VX>};
                false ->
                    case beamai_jinja_rt:truthy(<b>) of
                        true  -> {[<Y>], <VY>};
                        false -> {[<Z>], <VZ>}
                    end
            end
...
```erlang
%% {% for x in xs %}...{% endfor %}
[ begin V1 = beamai_jinja_rt:bind(V, x, X__), [...] end
  || X__ <- beamai_jinja_rt:to_list(<xs>) ]
```
...
{_, Acc} = lists:foldl(
    fun(X__, {N__, A__}) ->
        L__ = beamai_jinja_rt:loop(N__, Len__, X__, Items__),
        V1 = beamai_jinja_rt:bind2(V, x, X__, loop, L__),
        {N__ + 1, [[...] | A__]}
    end, {1, []}, Items__),
lists:reverse(Acc)
```
...
```erlang
%% {% set a = E %}          →  V1 = beamai_jinja_rt:bind(V0, a, <E>)
%% {% set a %}B{% endset %} →  V1 = beamai_jinja_rt:bind(V0, a, {safe, [<B>]})
%% {% with a = E %}B{% endwith %}
%%     →  begin V1 = beamai_jinja_rt:push(V0, #{a => <E>}), [<B>] end
%% {% filter f %}B{% endfilter %}
%%     →  beamai_jinja_filters:f({safe, [<B>]}, #{})
```
...
```erlang
%% {% block item %}        —— 只传模板级作用域
(maps:get(item, B, Default))(beamai_jinja_rt:globals(V), I, B)
...
前四条与 mustache 共享思路，可直接借鉴 `beamai_mustache_compiler:static_text/1`
与 `literal_of/1` 的实现。

## 8. `compile_inline/3`

与 mustache 一致（[04](04-codegen.md)、`beamai_mustache_compiler:compile_inline/3`）：
