...
- T31 `beamai_html_forms` —— forms 构造器
- T32 `beamai_html_text` / `beamai_html_escape` / `beamai_html_path` —— 文本、转义、路径
- T33 `beamai_html_engine` behaviour + `beamai_mustache_engine` 适配壳
...
- T35 `include/beamai_jinja.hrl` —— 数据契约
- T36 `beamai_jinja_scanner` —— 三种定界符、空白控制、raw
- T37 `beamai_jinja_lexer` —— 表达式词法
- T38 `beamai_jinja_expr` —— 表达式语法（优先级爬升）

**验收**：fixture 中所有「纯表达式」用例的 AST 断言通过；空白控制的组合矩阵全覆盖。

### 阶段 10 · 语句与 AST（T39–T40）

- T39 `beamai_jinja_parser` —— 语句结构、配对校验
- T40 `beamai_jinja_ast` —— 后处理 pass（文本合并、常量折叠、目标解析、依赖收集）

**验收**：所有语句形态的 AST 断言通过；配对错误的 8 类诊断各有用例。

### 阶段 11 · 运行时与内置库（T41–T44）

- T41 `beamai_jinja_rt` —— 作用域、真值、运算、迭代、loop、call
- T42 autoescape 与 safe 传播
- T43 `beamai_jinja_filters` —— 内置 filter 库
- T44 `beamai_jinja_tests` + `beamai_jinja_ext` —— 内置 test 与自定义注册

**验收**：filter/test 逐个有 fixture；safe 传播分类表每条有用例。

### 阶段 12 · 编译器（T45–T50）

- T45 `beamai_jinja_compiler` 骨架与模块契约
...
- T53 `beamai_jinja_transform`
- T54 `beamai_jinja_dev` + `beamai_jinja` 门面
...
`beamai_jinja_rt` 与内置库（T41–T44）不依赖 parser，可以与前端并行推进。
这是缩短工期最有效的一处。
