...
beamai_render ships two template engines side by side. They are peers: neither
...
classification as an annotation in `src/beamai_jinja_filters.erl`. A test asserts
...
```erlang
beamai_jinja_dev:check().        %% one-shot self test
beamai_jinja_dev:reload(all).    %% recompile and reload every stale template
beamai_html_dev:reload_all().    %% both engines
```
