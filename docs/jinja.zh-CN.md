...
beamai_render 同时提供两套模板引擎，二者是并列关系：谁也不替代谁，同一个项目里可以一起用。
...
并在 `src/beamai_jinja_filters.erl` 里以注解写在函数上。有一条测试**机械地**断言
...
```erlang
beamai_jinja_dev:check().        %% 一次性自检
beamai_jinja_dev:reload(all).    %% 重编并重载所有过期模板
beamai_html_dev:reload_all().    %% 两个引擎一起
```
