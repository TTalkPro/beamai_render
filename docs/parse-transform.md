...
`beamai_mustache_transform` is a parse_transform that handles three separate
...
| **(b)** Inline templates | `beamai_mustache:inline(~"Hi {{name}}!", Ctx)` | Expands the template in place at compile time | Same output, parsed and interpreted at run time |
...
%% {{@ hello}} becomes:
%%   case beamai_mustache_rt:lookup([locale], S) of
%%       <<"en">> -> <<"Hello">>;
%%       <<"fr">> -> <<"Bonjour">>;
%%       _        -> <<"hello">>
%%   end
compile_tag($@, [Key], _Body, Opts) ->
    A = beamai_mustache_ext:anno(Opts),
    Table = beamai_mustache_ext:ext_opt(?MODULE, Opts, table()),
    ...
    {'case', A, beamai_mustache_ext:lookup_expr([locale], Opts), Clauses}.
...
its name must come from `Opts`. `beamai_mustache_ext` provides builders so you
...
| `lookup_expr(Keys, Opts)` | `beamai_mustache_rt:lookup(Keys, S)` |
| `escape_expr(Expr, Opts)` | `beamai_mustache_rt:escape(Expr)` |
| `to_binary_expr(Expr, Opts)` | `beamai_mustache_rt:to_binary(Expr)` |
...
- the module does not exist, or does not `-behaviour(beamai_mustache_ext)`
...
hi(Name, Locale) ->
    beamai_mustache:inline(~"{{@hello}}, {{name}}!",
...
Extension validation failures come from `beamai_mustache_ext` and are reported the
...
`beamai_jinja_transform` is the same three forms for the other engine, and the two
can be applied to one module:

```erlang
-module(my_views).
-compile({parse_transform, beamai_mustache_transform}).
-compile({parse_transform, beamai_jinja_transform}).
...
| **(b)** inline | `beamai_mustache:inline(~"...", Ctx)` | `beamai_jinja:inline(~"...", Ctx)` |
