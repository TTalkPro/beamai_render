...
`beamai_mustache_transform` 是一个 parse_transform，承担三件互不相干的事。按模块启用：
...
| **(b)** 内联模板 | `beamai_mustache:inline(~"Hi {{name}}!", Ctx)` | 编译期就地展开 | 输出相同，但每次调用都在运行期解析并解释 |
...
模板被内联展开时，context stack 的变量会被改名，所以变量名必须从 `Opts` 里取。`beamai_mustache_ext` 提供了构造函数，常见场景都不用碰 abstract forms：
...
- 模块不存在，或没写 `-behaviour(beamai_mustache_ext)`
...
扩展校验失败由 `beamai_mustache_ext` 报出，走同一通道：`not_an_ext_module`、`ext_missing_callback`、`marker_reserved`、`marker_conflict` 等。

---

## jinja 的 transform

`beamai_jinja_transform` 是同样的三种形态，只是换了引擎；两个 transform 可以用在同一个模块上：
...
| **(b)** 内联 | `beamai_mustache:inline(~"...", Ctx)` | `beamai_jinja:inline(~"...", Ctx)` |
