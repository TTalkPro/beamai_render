%%%-------------------------------------------------------------------
%%% @doc Worked example for beamai_render.
%%%
%%% There is no application to start, no bootstrap call, no ets table and no
%%% process: beamai_render is a library application and the templates in this
%%% directory are turned into modules (view_complex, view_shared_item, ...)
%%% by rebar3_beamai_render during `rebar3 compile'.
%%%
...
start_via_facade() ->
    Mod = view_complex,
    io:format("~ts~n", [beamai_mustache:render(Mod, context())]).
