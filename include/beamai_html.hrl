%%%-------------------------------------------------------------------
%%% beamai_render -- the part of the data contract that is engine-neutral.
%%%
%%% Only what the mustache and jinja front ends genuinely share lives here.
%%% Engine-specific node, token and option types belong in beamai_mustache.hrl and
%%% beamai_jinja.hrl respectively; the moment this header starts collecting them it
...
%%%-------------------------------------------------------------------
-ifndef(BEAMAI_HTML_HRL).
-define(BEAMAI_HTML_HRL, true).
...
-type beamai_html_loc() :: {Line :: pos_integer(), Col :: pos_integer()}.

%% Template bodies and paths are UTF-8 binaries throughout; beamai_html_text is
%% the only place a conversion happens.
-type beamai_html_path() :: binary().
...
-type beamai_html_error(Reason) ::
        {error, {File :: beamai_html_path(), Line :: pos_integer(), Reason}}.
...
-define(BEAMAI_HTML_MUSTACHE_KEYS,
        [views, suffix, out_dir, prefix, extensions, ext_opts, line_map,
         warnings_as_errors]).

-define(BEAMAI_HTML_JINJA_KEYS,
