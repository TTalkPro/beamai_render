...
%% path. The layout is fixed -- <out>/rebar3_beamai_render/ebin and <out>/src -- so
%% the sibling can be derived from this module's own beam, in both the
%% standalone build and the installed-plugin build.
%%
%% If the modules are already loadable, nothing is touched: an beamai_render already
...
add_core_path() ->
    case code:which(?MODULE) of
        Beam when is_list(Beam) ->
            Ebin = filename:dirname(Beam),                 % .../rebar3_beamai_render/ebin
            AppDir = filename:dirname(Ebin),               % .../rebar3_beamai_render
