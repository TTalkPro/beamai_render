...
%%% @end
%%%-------------------------------------------------------------------
-module(rebar3_beamai_render_emit).
...
%% resolve partials; see rebar3_beamai_render_check:partials_exist/3 for why the
...
render(#tpl{source = Body, rel_path = Rel, module = Mod, stamp = Stamp,
    ...
    case Engine:parse(Body, EOpts) of
        {error, _} = E -> E;
        {ok, Nodes} ->
            case rebar3_beamai_render_check:partials_exist(Nodes, Available, Rel, Opts) of
...
text(Forms, Rel, Engine) ->
    ...
    %% default. beamai_mustache_compiler emits non-ASCII static text as
...
do_write(Path0, Bin) ->
    ...
    %% rebar3_beamai_render_gc on the next run.
