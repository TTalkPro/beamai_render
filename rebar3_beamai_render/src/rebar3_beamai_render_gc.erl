...
%%% @end
%%%-------------------------------------------------------------------
-module(rebar3_beamai_render_gc).
...
remove(File, Engine, EbinDir, AppDir) ->
    case classify(File, Engine) of
        foreign ->
            rebar_api:warn("beamai_render: ~ts is not a generated file and was left "
        ...
        mine ->
            ...
            rebar_api:info("beamai_render: removing orphan ~ts", [rel(File, AppDir)]),
            _ = file:delete(File),
    ...
    case file:read_file(File) of
        ...
        {ok, Bin} ->
            ...
            case banner_of(Bin) of
                ...
                none ->
                    case rebar3_beamai_render_scan:meta(File, Engine) of
                        error -> foreign;
                        _     -> mine
                    end
            end
