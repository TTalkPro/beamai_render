...
%%% @end
%%%-------------------------------------------------------------------
-module(rebar3_beamai_render_migrate).
...
do(State) ->
    ...
    Write andalso
        rebar_api:warn("mustache migrate: --write rewrites your templates in "
                       ...
                       "is committed", []),
    try
        ok = rebar3_beamai_render_prv:require_core(State),
        Totals = lists:foldl(
        ...
        throw:{rebar3_beamai_render, Reason} -> {error, {?MODULE, Reason}}
    end.
...
app(AppInfo, State, Write, Verbose, Acc) ->
    case rebar3_beamai_render_opts:for_app(AppInfo, State, beamai_mustache_engine) of
        skip       -> Acc;
        {ok, Opts} ->
            ...
            Tpls = rebar3_beamai_render_scan:templates(Opts),
...
one(#tpl{abs_path = Abs, rel_path = Rel, source = Body, module = Mod},
    ...
    case rebar3_beamai_render_rewrite:file(Body, Opts) of
        {error, {File, Line, Reason}} ->
            throw({rebar3_beamai_render, {errors, [{File, Line, Reason}]}});
        {Body, 0, []} ->
...
emit(Abs, Rel, Old, New, false) ->
    _ = Abs,
    io:format("~ts", [rebar3_beamai_render_diff:unified(Rel, Old, New)]),
    ok;
emit(Abs, _Rel, _Old, New, true) ->
    ...
    case rebar3_beamai_render_emit:write(Abs, New) of
