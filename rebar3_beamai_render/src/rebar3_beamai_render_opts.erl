...
%%% @end
%%%-------------------------------------------------------------------
-module(rebar3_beamai_render_opts).
...
normalize(Raw, AppDir, AppInfo, Engine) ->
    ...
    #mopts{engine     = Engine,
           ...
           %%
           %% `views' is deliberately absent. beamai_mustache_ast:resolve_partials/2
           ...
           %% for an umbrella app. rebar3_beamai_render_check does that check instead,
...
check_unknown(Raw, Wae, Engine) ->
    ...
    case [K || {K, _} <- Raw, not lists:member(K, Known)] of
        ...
        Unknown ->
            ...
            case Wae of
                true  -> throw({rebar3_beamai_render, {bad_opts, lists:flatten(Msg)}});
...
string_opt(Key, Raw, Default) ->
    case proplists:get_value(Key, Raw, Default) of
        ...
        L when is_list(L)   -> L;
        Other -> throw({rebar3_beamai_render, {bad_opt, Key, Other}})
    end.
...
%% letter -- rebar3_beamai_render_name enforces that half.
prefix(Raw, Engine) ->
    ...
    case valid_prefix(P) of
        ...
        false -> throw({rebar3_beamai_render, {bad_prefix, P}})
    end.
...
extensions(Raw) ->
    ...
    is_list(Mods) orelse throw({rebar3_beamai_render, {bad_opt, extensions, Mods}}),
    lists:foreach(
      fun(M) when is_atom(M) ->
              case code:ensure_loaded(M) of
                  {module, M} -> ok;
                  {error, R}  -> throw({rebar3_beamai_render, {ext_not_loadable, M, R}})
              end;
         (Other) ->
              throw({rebar3_beamai_render, {bad_opt, extensions, Other}})
      end, Mods),
    Mods.
...
ext_opts(Raw) ->
    case proplists:get_value(ext_opts, Raw, #{}) of
        ...
        Other -> throw({rebar3_beamai_render, {bad_opt, ext_opts, Other}})
    end.
