...
%%% beamai_mustache_compiler:source_hash/2 produces, read back out of the
...
%%% consulted is rebar3_beamai_render_stale, for reasons documented there.
%%% @end
%%%-------------------------------------------------------------------
...
-include("rebar3_beamai_render.hrl").
...
do(State) ->
    case rebar_state:command_args(State) of
        ["migrate" | _] -> rebar3_beamai_render_migrate:do(State);
        _ ->
            {Args, _} = rebar_state:command_parsed_args(State),
            Force = proplists:get_value(force, Args, false),
            run(State, beamai_mustache_engine, Force)
    end.
...
run(State, Engine, Force) ->
    try
        ...
        throw:{rebar3_beamai_render, Reason} ->
            {error, {?MODULE, Reason}}
    end.

%% @doc Make the compile core reachable, or explain why it is not.
%%
%% Exported because rebar3_beamai_render_migrate needs the same guarantee.
...
%% rebar3_beamai_render:init/1 and the plugin's rebar.config); when it has been
%% repackaged without it, fall back to the beamai_render the project already depends
%% on -- it has to be there anyway, since the generated modules call
%% beamai_mustache_rt at run time. Failing that, say so in terms a user can act on
%% rather than dying on an undef halfway down the pipeline.
require_core(State) ->
    case code:ensure_loaded(beamai_mustache_compiler) of
        {module, _} -> ok;
...
borrow_core(State) ->
    ...
    Declared = [rebar_app_info:ebin_dir(A)
                || A <- Apps, rebar_app_info:name(A) =:= <<"beamai_render">>],
    ...
    %% beamai_render, whether it arrived as a dependency, a checkout or a plugin.
    Base = rebar_dir:base_dir(State),
    Globbed = lists:append(
                [filelib:wildcard(filename:join([Base, Kind, "beamai_render", "ebin"]))
    ...
    case code:ensure_loaded(beamai_mustache_compiler) of
        {module, _} -> ok;
        {error, R}  -> throw({rebar3_beamai_render, {no_core, R}})
    end.
...
collect(State, Engine) ->
    lists:foldr(
      fun(App, Acc) ->
              case rebar3_beamai_render_opts:for_app(App, State, Engine) of
                  skip ->
                      %% Most apps have no templates; saying so on every run
                      %% would be noise.
                      Acc;
                  {ok, Opts} ->
                      [{App, Opts, rebar3_beamai_render_scan:templates(Opts)} | Acc]
              end
...
validate(Collected, State) ->
    ...
    case Errors of
        [] -> State1;
        _  -> throw({rebar3_beamai_render, {errors, Errors}})
    end.

-define(MODULE_TABLE, {beamai_render, modules}).
...
app_errors(Opts, Tpls) ->
    Names = [{T, rebar3_beamai_render_name:validate(T#tpl.module, T#tpl.rel_path, Opts)}
             || T <- Tpls],
    [E || {_, {error, E}} <- Names]
        ++ rebar3_beamai_render_check:collisions(Tpls, Opts)
        ++ rebar3_beamai_render_check:handwritten(Tpls, Opts).
...
cross_app_warnings(Collected, State) ->
    ...
    case rebar3_beamai_render_check:cross_app(PerApp) of
        [] -> ok;
        Dups ->
            ...
            Msg = lists:flatten(
                    ["module name(s) generated more than once; give them "
                     ...
                     lists:join("; ", Lines)]),
            case Wae of
                true  -> throw({rebar3_beamai_render, {bad_opts, Msg}});
                false -> rebar_api:warn("beamai_render: ~s", [Msg]), ok
            end
    end.
...
emit_app({_App, Opts, Tpls}, Force, {C0, S0, K0}) ->
    ...
    Forced = rebar3_beamai_render_check:stale_partials(Tpls, Opts),
    ...
    case Errors of
        [] -> ok;
        _  -> throw({rebar3_beamai_render, {errors, lists:reverse(Errors)}})
    end,
    Cleaned = rebar3_beamai_render_gc:sweep(Tpls, Opts),
    {Touched, StaleErrs} = rebar3_beamai_render_stale:touch(Opts, []),
    case StaleErrs of
        [] -> ok;
        _  -> throw({rebar3_beamai_render, {errors, lists:reverse(StaleErrs)}})
    end,
    Touched > 0 andalso
        rebar_api:debug("beamai_render: touched ~b module(s) with a stale template "
                        "attribute", [Touched]),
...
one(T = #tpl{module = Mod, rel_path = Rel, out_path = Out, stamp = Stamp},
    ...
    case needs_build(Force, sets:is_element(Mod, Forced), Out, Stamp, Engine) of
        false ->
            rebar_api:debug("beamai_render: ~ts is up to date", [Rel]),
            {C, S + 1, Errs};
        true ->
            rebar_api:debug("beamai_render: compiling ~ts -> ~ts", [Rel, Out]),
            case rebar3_beamai_render_emit:render(T, Opts, Available) of
                {error, {File, Line, Reason}} ->
                    {C, S, [{File, Line, Reason} | Errs]};
                {errors, Es} ->
                    {C, S, lists:reverse(Es) ++ Errs};
                {ok, Bin} ->
                    case rebar3_beamai_render_emit:write(Out, Bin) of
                        {error, R}  -> throw({rebar3_beamai_render, R});
...
needs_build(_Force, _Forced, Out, Stamp, Engine)   ->
    ...
    rebar3_beamai_render_scan:stamp_of(Out, Engine) =/= {ok, Stamp}.
...
format_error({no_core, R}) ->
    lists:flatten(io_lib:format(
                    "the beamai_render compile core is not on the code path (~p). "
                    "The plugin normally carries its own copy; failing that it "
                    "borrows the beamai_render your project depends on, which needs "
                    "to have been built at least once -- try `rebar3 compile\' "
                    "first, and check that beamai_render is in your deps", [R]));
