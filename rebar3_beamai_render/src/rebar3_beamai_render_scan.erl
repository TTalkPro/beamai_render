...
%%%     beamai_mustache_compiler:source_hash/2 computes for it;
...
describe(Abs, Opts = #mopts{app_dir = AppDir, views_dir = ViewsDir,
                            ...
                            engine_opts = CO}) ->
    Name = rebar3_beamai_render_name:name_of(Abs, ViewsDir, Suffix),
    Mod  = rebar3_beamai_render_name:module_of(Name, Opts),
    Rel  = relative(Abs, AppDir),
    case file:read_file(Abs) of
        {error, Posix} ->
            throw({rebar3_beamai_render, {errors, [{Rel, 0, {read_failed, Posix}}]}});
        {ok, Body} ->
            #tpl{abs_path = Abs,
                 rel_path = Rel,
                 name     = Name,
                 module   = Mod,
                 out_path = rebar3_beamai_render_name:out_file(Mod, Opts),
                 source   = Body,
                 %% The one and only place a stamp comes from. Computing it
                 %% here instead of in the plugin keeps the plugin and
                 %% beamai_mustache_dev from ever disagreeing about freshness.
