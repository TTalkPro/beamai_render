...
%%% through beamai_html_path, which each engine's parse_transform also uses. That
...
%%% @end
%%%-------------------------------------------------------------------
-module(rebar3_beamai_render_stale).
        ...
        Attrs ->
            ...
            lists:foldl(
              fun({_Name, Path}, {N1, E1}) ->
                      case beamai_html_path:resolve(Path, dirs(Erl, Opts, IncDirs)) of
                          {error, {not_found, _}} ->
                              %% Line 0: beamai_html_path:scan/1 works on the
...
maybe_touch(Tpl, Erl) ->
    case {mtime(Tpl), mtime(Erl)} of
        {T, E} when T > E, T > 0 ->
            rebar_api:debug("beamai_render: touching ~ts (template ~ts is newer)",
                            [Erl, Tpl]),
    ...
    case file:read_file(File) of
        ...
        {ok, Bin} ->
            case binary:match(Bin, atom_to_binary(Attr, utf8)) of
                nomatch -> [];
                _ ->
                    case beamai_html_path:scan(File, Attr) of
