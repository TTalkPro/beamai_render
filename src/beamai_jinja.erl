...
render_string(Template, Ctx, Opts) ->
    case compile_set(Template, Opts) of
        ...
        {error, _} = E -> erlang:error({beamai_jinja, E})
    end.

%% @doc Render an inline template.
%%
%% With beamai_jinja_transform applied and a literal first argument, this call is
...
%% (beamai_jinja_compiler:inline_forbidden/1).
-spec inline(binary(), term()) -> binary().
inline(Template, Ctx) ->
    Opts = #{source => <<"inline">>},
    case beamai_jinja_parser:parse(Template, Opts) of
        {error, _} = E ->
            erlang:error({beamai_jinja, E});
        {ok, Nodes} ->
            case forbidden_in(Nodes) of
                {yes, Kind} ->
                    erlang:error({beamai_jinja,
                                  {error, {<<"inline">>, 1,
...
forbidden_in([Node | Rest]) ->
    Kind = element(1, Node),
    case beamai_jinja_compiler:inline_forbidden(Kind) of
        true  -> {yes, Kind};
        false ->
            case forbidden_bodies(beamai_jinja_ast:bodies(Node)) of
...
prefix_for(Template, Templates, Opts) ->
    Digest = erlang:md5(term_to_binary(
                          {Template, lists:sort(maps:to_list(Templates)),
                           ...
                           beamai_jinja_compiler:normalize_opts(Opts),
                           ?BEAMAI_JINJA_VSN})),
...
compile_all(Template, Templates, Prefix, Main, Opts) ->
    ...
    Units = [{Main, <<"main">>, Template}
             | [{beamai_jinja_ast:module_name(N, Base), N, T}
...
parse_units([{Mod, Name, Source} | Rest], Base, Acc) ->
    ...
    case beamai_jinja_parser:parse(Source, Opts) of
        {error, _} = E -> E;
...
unit_opts(Mod, Name, Source, Base) ->
    Base#{module => Mod, source => Name,
          stamp => beamai_jinja_compiler:source_hash(Source, Base)}.
