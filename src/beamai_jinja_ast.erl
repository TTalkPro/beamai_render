...
%%% the same error.
%%%
%%% Constant folding evaluates through beamai_jinja_rt, never through a second
%%% implementation. If the two disagreed, `{{ 1 + 2 }}' and `{{ a + b }}'
%%% would answer differently for the same values.
%%% @end
%%%-------------------------------------------------------------------
-module(beamai_jinja_ast).
...
-spec postprocess(nodes(), map()) ->
          {ok, nodes(), [module()]} | beamai_jinja_error().
...
default_prefix() -> ?BEAMAI_JINJA_DEFAULT_PREFIX.
...
module_name(Name, Opts) ->
    ...
    Suffix = maps:get(suffix, Opts, ?BEAMAI_JINJA_DEFAULT_SUFFIX),
...
-spec map_body(fun((nodes()) -> nodes()), beamai_jinja_node()) -> beamai_jinja_node().
...
-spec bodies(beamai_jinja_node()) -> [nodes()].
...
prune_branches(L, [{{lit, _, V}, Body} | Rest], Else, Kept) ->
    case beamai_jinja_rt:truthy(V) of
        true when Kept =:= [] -> Body;
...
-spec fold_expr(beamai_jinja_expr()) -> beamai_jinja_expr().
fold_expr({binop, L, Op, A0, B0}) ->
    A = fold_expr(A0), B = fold_expr(B0),
    case {A, B} of
        {{lit, _, VA}, {lit, _, VB}} -> try_lit(L, fun() -> beamai_jinja_rt:binop(Op, VA, VB) end,
                                                {binop, L, Op, A, B});
        _ -> {binop, L, Op, A, B}
    end;
fold_expr({unop, L, 'not', E0}) ->
    case fold_expr(E0) of
        {lit, _, V} -> {lit, L, not beamai_jinja_rt:truthy(V)};
        E           -> {unop, L, 'not', E}
    end;
...
fold_expr({'and', L, A0, B0}) ->
    A = fold_expr(A0), B = fold_expr(B0),
    case A of
        {lit, _, V} -> case beamai_jinja_rt:truthy(V) of true -> B; false -> A end;
        _           -> {'and', L, A, B}
    end;
fold_expr({'or', L, A0, B0}) ->
    A = fold_expr(A0), B = fold_expr(B0),
    case A of
        {lit, _, V} -> case beamai_jinja_rt:truthy(V) of true -> A; false -> B end;
        _           -> {'or', L, A, B}
    end;
fold_expr({'cond', L, C0, T0, E0}) ->
    ...
    case C of
        {lit, _, V} ->
            case {beamai_jinja_rt:truthy(V), E} of
                {true, _}          -> T;
...
foldable_filter(Name, {lit, _, _}, {Pos, Kw, undefined, undefined}) ->
    not lists:member(Name, ?IMPURE_FILTERS)
        ...
        andalso beamai_jinja_ext:lookup(filter, Name) =/= error;
foldable_filter(_Name, _E, _Args) ->
    false.

eval_filter(Name, {lit, _, V}, {Pos, Kw, _, _}) ->
    {ok, {M, F}} = beamai_jinja_ext:lookup(filter, Name),
    Params = maps:get(Name, beamai_jinja_filters:params(), []),
    M:F(V, beamai_jinja_args:build(Params, [X || {lit, _, X} <- Pos],
                               [{N, X} || {N, {lit, _, X}} <- Kw])).
...
resolve_targets(Nodes, Opts) ->
    try
        ...
        throw:{beamai_jinja_ast, Loc, Reason} ->
            {error, {beamai_html_text:source(Opts), element(1, Loc), Reason}}
    end.
...
resolve_node({extends, L, T}, Opts, Deps) ->
    ...
    case Mod =:= maps:get(module, Opts, undefined) of
        true  -> throw({beamai_jinja_ast, L, {extends_cycle, [Mod]}});
...
target(Name, L, Opts, Deps, IgnoreMissing) when is_binary(Name) ->
    ...
    case IgnoreMissing orelse exists(Name, Opts) of
        true  -> {Mod, [Mod | Deps]};
        false -> throw({beamai_jinja_ast, L, {template_not_found, Name}})
    end;
target(_Expr, L, _Opts, _Deps, _Ign) ->
    throw({beamai_jinja_ast, L, {dynamic_target_unsupported, L}}).
