...
%%% @end
%%%-------------------------------------------------------------------
-module(beamai_jinja_compiler).

-include("beamai_jinja.hrl").
...
-define(THROW(Loc, Reason), throw({beamai_jinja_compile, Loc, Reason})).

%%%===================================================================
%%% API
%%%===================================================================

-spec forms([beamai_jinja_node()], map()) ->
          {ok, [erl_parse:abstract_form()], [module()]} | beamai_jinja_error().
forms(Nodes0, Opts) ->
    case beamai_jinja_ast:postprocess(Nodes0, Opts) of
        {error, _} = E -> E;
        {ok, Nodes, Deps} ->
            case new_state(Opts) of
                {error, Reason} ->
                    {error, {beamai_html_text:source(Opts), 1, Reason}};
                {ok, St0} ->
                    try
                        St1 = collect(Nodes, St0),
                        module_forms(Nodes, Deps, St1)
                    catch
                        throw:{beamai_jinja_compile, Loc, Reason} ->
                            {error, {beamai_html_text:source(Opts), line_of(Loc), Reason}}
                    end
            end
    end.

%% @doc Compile a template into one expression, for the parse_transform.
-spec compile_inline([beamai_jinja_node()], erl_parse:abstract_expr(), map()) ->
          {ok, erl_parse:abstract_expr()} | beamai_jinja_error().
compile_inline(Nodes0, CtxExpr, Opts) ->
    Nodes = beamai_jinja_ast:merge_text(beamai_jinja_ast:fold_constants(
                                      beamai_jinja_ast:merge_text(Nodes0))),
    case new_state(Opts) of
        {error, Reason} ->
            {error, {beamai_html_text:source(Opts), 1, Reason}};
        {ok, St0} ->
...
source_hash(Body, Opts) ->
    erlang:md5(term_to_binary({Body, normalize_opts(Opts), ?BEAMAI_JINJA_VSN},
                              [deterministic])).
...
new_state(Opts0) ->
    ...
    case beamai_jinja_ext:registry(maps:get(extensions, Opts, [])) of
        {error, _} = E -> E;
        {ok, Reg} ->
            {ok, #cs{opts     = Opts,
...
collect_node({from, Loc, Mod, Names, WithCtx}, St) ->
    ...
    %% beamai_jinja:render_string/3 both do -- a typo here is a build error rather
...
collect_node(Node, St) ->
    lists:foldl(fun(B, S) -> collect(B, S) end, St, beamai_jinja_ast:bodies(Node)).
...
module_forms(Nodes, Deps, St0) ->
    ...
    Allowed = [erlang, lists, maps, code, beamai_jinja_rt, beamai_jinja_filters,
               beamai_jinja_tests, beamai_html_escape, Mod]
...
source_attr(#cs{opts = Opts}) ->
    Attr = #{path  => beamai_html_text:source(Opts),
             stamp => maps:get(stamp, Opts, <<>>),
             mtime => maps:get(mtime, Opts, 0),
             vsn   => ?BEAMAI_JINJA_VSN,
...
underscore_unused({var, L, V}, Following) ->
    case hd(atom_to_list(V)) =/= $_ andalso not beamai_html_forms:uses_var(Following, V) of
...
emit_macro(Name, Loc, Params, Body, St) ->
    ...
    Extra = [{map_field_assoc, L, a(L, varargs),
             ...
             {map_field_assoc, L, a(L, kwargs),
              rt(L, kwargs, [v(L, ArgsVar), beamai_html_forms:atoms(L, Names)])},
...
expr({name, Loc, N}, Scope, St) ->
    L = line(Loc, St),
    Fun = case beamai_jinja_rt:is_builtin(N) of
    ...
    case maps:get(N, St#cs.imports, undefined) of
        ...
        undefined ->
            case lists:keyfind(N, 1, St#cs.macros) of
                ...
                false ->
                    case beamai_jinja_rt:is_builtin(N) of
                        true  -> builtin;
                        false -> none
                    end
            end;
...
test_params(_St) -> beamai_jinja_tests:params().
...
apply_body_filter(Chain, BodyExpr, Scope, St, _L) ->
    Placeholder = beamai_jinja_parser:body_placeholder(),
    {Expr, St1} = expr(Chain, Scope, St),
    {substitute_placeholder(Expr, Placeholder, BodyExpr), St1}.

substitute_placeholder({call, _, {remote, _, {atom, _, beamai_jinja_rt},
                                 {atom, _, resolve}},
...
reject_inline_node(Node) ->
    ...
    case inline_forbidden(K) of
        ...
        false -> lists:foreach(fun reject_inline/1, beamai_jinja_ast:bodies(Node))
    end.
...
rt(L, F, Args) -> rem_call(L, beamai_jinja_rt, F, Args).
