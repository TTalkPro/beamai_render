...
%%%
%%% It is a near-copy of beamai_mustache_dev, and deliberately not a shared module
...
%%% engine argument. beamai_html_dev:reload_all/0 is the one place the two meet.
%%% @end
%%%-------------------------------------------------------------------
-module(beamai_jinja_dev).
...
check() ->
    case code:ensure_loaded(beamai_jinja_compiler) of
        {error, R} ->
...
-spec source(module()) -> {ok, beamai_jinja_source()} | {error, not_a_template_module}.
source(Mod) ->
...
%% beamai_jinja_compiler:source_hash/2 the plugin uses. Two independent
...
-spec stale_by_stamp(binary() | string(), binary(), beamai_jinja_source()) ->
          boolean() | {error, term()}.
stale_by_stamp(Path, Stamp, #{opts := Opts}) ->
    case file:read_file(Path) of
        {error, R} -> {error, {R, Path}};
        {ok, Body} -> beamai_jinja_compiler:source_hash(Body, Opts) =/= Stamp
    end.
...
rebuild(Mod, Path, Body, Recorded) ->
    ...
    case beamai_jinja_parser:parse(Body, Opts) of
        {error, _} = E -> E;
        {ok, Ast} ->
            case beamai_jinja_compiler:forms(Ast, Opts) of
                {error, _} = E     -> E;
