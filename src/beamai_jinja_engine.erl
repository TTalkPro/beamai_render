%%%-------------------------------------------------------------------
%%% @doc beamai_html_engine adapter for the jinja engine.
%%%
...
parse(Body, Opts) -> beamai_jinja_parser:parse(Body, Opts).

-spec forms(term(), map()) ->
          {ok, [erl_parse:abstract_form()], [module()]} | {error, term()}.
forms(Ast, Opts) -> beamai_jinja_compiler:forms(Ast, Opts).

-spec source_hash(binary(), map() | [{atom(), term()}]) -> binary().
source_hash(Body, Opts) -> beamai_jinja_compiler:source_hash(Body, Opts).

-spec module_name(binary(), map()) -> module().
module_name(Name, Opts) -> beamai_jinja_ast:module_name(Name, Opts).
...
default_prefix() -> beamai_jinja_ast:default_prefix().

-spec config_key() -> atom().
config_key() -> jinja_opts.

-spec known_keys() -> [atom()].
known_keys() -> ?BEAMAI_HTML_JINJA_KEYS.
