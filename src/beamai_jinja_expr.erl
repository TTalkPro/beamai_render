...
-type toks() :: [beamai_jinja_expr_token()].
-type expr() :: beamai_jinja_expr().
-type loc()  :: beamai_html_loc().

-define(THROW(Loc, Reason), throw({beamai_jinja_expr, Loc, Reason})).
...
-spec parse(toks()) -> {ok, expr(), toks()} | {error, loc(), beamai_jinja_reason()}.
parse(Toks) ->
    try condexpr(Toks) of
        {E, Rest} -> {ok, E, Rest}
    catch
        throw:{beamai_jinja_expr, Loc, Reason} -> {error, Loc, Reason}
    end.

%% @doc One expression that must consume every token.
-spec parse_all(toks()) -> {ok, expr()} | {error, loc(), beamai_jinja_reason()}.
parse_all(Toks) ->
    case parse(Toks) of
        {ok, E, []}          -> {ok, E};
...
-spec parse_list(toks()) -> {ok, [expr()], toks()} | {error, loc(), beamai_jinja_reason()}.
parse_list(Toks) ->
    try list_of(Toks, []) of
        {Es, Rest} -> {ok, Es, Rest}
    catch
...
-spec parse_args(toks()) ->
          {ok, beamai_jinja_args(), toks()} | {error, loc(), beamai_jinja_reason()}.
parse_args(Toks) ->
    try args(Toks) of
        {A, Rest} -> {ok, A, Rest}
    catch
...
-spec expect_end(toks()) -> ok | {error, loc(), beamai_jinja_reason()}.
expect_end([])      -> ok;
