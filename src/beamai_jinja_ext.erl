...
-spec registry([module()]) -> {ok, registry()} | {error, beamai_jinja_reason()}.
registry(Mods) ->
    Base = #{filters => builtin(filter),
             tests   => builtin(test),
             params  => beamai_jinja_filters:params(),
             modules => []},
    try {ok, lists:foldl(fun add/2, Base, Mods)}
    catch throw:{beamai_jinja_ext, Reason} -> {error, Reason} end.
...
call_table(Mod, Fun) ->
    case erlang:function_exported(Mod, Fun, 0) of
        true ->
            case Mod:Fun() of
                M when is_map(M) -> M;
                _ -> throw({beamai_jinja_ext, {filter_name_conflict, Fun, Mod}})
            end;
        false ->
            #{}
    end.
...
builtin(filter) -> beamai_jinja_filters:filters();
builtin(test)   -> beamai_jinja_tests:tests().
...
-spec spec_module(module()) ->
          {ok, #{filters := [atom()], tests := [atom()]}} | {error, beamai_jinja_reason()}.
spec_module(Mod) ->
...
-spec format_error(beamai_jinja_reason()) -> string().
format_error({filter_name_conflict, Name, Mod}) ->
    lists:flatten(io_lib:format(
                    "~p already names a builtin filter or test, and ~p tries "
                    "to register it again. Rename the one in ~p; beamai_render does "
