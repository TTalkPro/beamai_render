...
striptags(V, _A) ->
    B = to_bin(beamai_jinja_rt:unsafe(V)),
    Text = strip_tags(B, <<>>, false),
    beamai_jinja_rt:safe(collapse_ws(Text)).
...
wordcount(V, _A) ->
    erlang:length([W || W <- binary:split(to_bin(beamai_jinja_rt:unsafe(V)),
                                          [<<" ">>, <<"\n">>, <<"\t">>, <<"\r">>],
                                          [global]), W =/= <<>>]).
...
format(V, A) ->
    Fmt = to_bin(beamai_jinja_rt:unsafe(V)),
    Args = maps:get('$positional', A, []),
    beamai_jinja_rt:safe(printf(Fmt, Args, <<>>)).
...
fmt_one($s, A) -> beamai_jinja_rt:to_binary(A);
fmt_one($r, A) -> beamai_jinja_rt:repr(A);
...
urlencode(V, _A) ->
    B = beamai_jinja_rt:unsafe(V),
    beamai_jinja_rt:safe(
      case B of
          M when is_map(M) ->
              iolist_to_binary(
                lists:join(<<"&">>,
                           [<<(pct(atom_to_binary(K, utf8)))/binary, "=",
                              (pct(beamai_jinja_rt:to_binary(Val)))/binary>>
...
urlize_word(W) ->
    case is_url(W) of
        true  -> Esc = beamai_html_escape:escape(W, ?TAG),
                 [<<"<a href=\"">>, Esc, <<"\" rel=\"noopener\">">>, Esc, <<"</a>">>];
        false -> beamai_html_escape:escape(W, ?TAG)
    end.
...
escape(V, _A) -> beamai_jinja_rt:safe(iolist_to_binary(beamai_jinja_rt:escape(V))).
%% @safe generating
...
forceescape(V, _A) ->
    %% beamai_html_escape:escape/2 returns iodata, and beamai_jinja_rt:safe/1 renders
    %% whatever it is handed -- a raw list would come out as a Python list
    %% repr, so it is flattened here.
    beamai_jinja_rt:safe(
      iolist_to_binary(
        beamai_html_escape:escape(beamai_jinja_rt:to_binary(beamai_jinja_rt:unsafe(V)),
                              ?TAG, <<"&#34;">>))).

%% @safe generating
-spec safe(term(), map()) -> term().
safe(V, _A) -> beamai_jinja_rt:safe(V).

%% @safe escaping
-spec string(term(), map()) -> term().
string(V, _A) -> beamai_jinja_rt:to_binary(beamai_jinja_rt:unsafe(V)).

%% @safe escaping
-spec pprint(term(), map()) -> term().
pprint(V, _A) -> beamai_jinja_rt:repr(beamai_jinja_rt:unsafe(V)).
...
join(V, A) ->
    ...
    Items = case A of
                #{attribute := At} when At =/= undefined ->
                    [beamai_jinja_rt:attr(E, atomize(At)) || E <- seq(V)];
                _ -> seq(V)
            end,
    Parts = [beamai_jinja_rt:escape(E) || E <- Items],
    beamai_jinja_rt:safe(iolist_to_binary(lists:join(beamai_jinja_rt:escape(Sep), Parts))).
...
reverse({safe, D}, A) ->
    beamai_jinja_rt:safe(reverse(iolist_to_binary(D), A));
reverse(V, _A) ->
    lists:reverse(seq(V)).
...
sort_key(E, Att, CS)       -> fold_case(beamai_jinja_rt:attr(E, atomize(Att)), CS).
...
sum(V, A) ->
    ...
    lists:foldl(fun(E, Acc) -> beamai_jinja_rt:add(Acc, E) end,
...
groupby(V, A) ->
    ...
    Pairs = [{key_or(beamai_jinja_rt:attr(E, Att), Default), E} || E <- seq(V)],
...
map(V, A) ->
    case A of
        #{attribute := Att} when Att =/= undefined ->
            [beamai_jinja_rt:attr(E, atomize(Att)) || E <- seq(V)];
        #{name := Name} when Name =/= undefined ->
            Extra = maps:get('$positional', A, []),
            [beamai_jinja_rt:apply_named(filter, atomize(Name), E, Extra)
             || E <- seq(V)];
        _ ->
            seq(V)
    end.
...
test_of(E, #{name := Name} = A) when Name =/= undefined ->
    Extra = maps:get('$positional', A, []),
    beamai_jinja_rt:truthy(beamai_jinja_rt:apply_named(test, atomize(Name), E, Extra));
test_of(E, _A) ->
    beamai_jinja_rt:truthy(E).

attr_test(E, #{attribute := Att} = A) ->
    V = beamai_jinja_rt:attr(E, atomize(Att)),
    case A of
        #{name := Name} when Name =/= undefined ->
            Extra = maps:get('$positional', A, []),
            beamai_jinja_rt:truthy(beamai_jinja_rt:apply_named(test, atomize(Name), V, Extra));
        _ ->
            beamai_jinja_rt:truthy(V)
    end.
...
items(V, _A) ->
    [{atom_to_binary(K, utf8), Val} || {K, Val} <- beamai_jinja_rt:items(as_map(V))].
...
attr(V, A) ->
    case beamai_jinja_rt:unsafe(V) of
...
tojson(V, _A) ->
    beamai_jinja_rt:safe(json_encode(beamai_jinja_rt:unsafe(V))).
...
default(V, A) ->
    ...
    case bool_arg(A, boolean, false) of
        true  -> case beamai_jinja_rt:truthy(V) of true -> V; false -> Fallback end;
...
bool_arg(A, K, D) ->
    case maps:get(K, A, D) of
        undefined -> D;
        V -> beamai_jinja_rt:truthy(V)
    end.

bin_arg(A, K, D) ->
    case maps:get(K, A, D) of
        undefined -> D;
        V -> beamai_jinja_rt:to_binary(beamai_jinja_rt:unsafe(V))
    end.
