...
%%% token and is rejected by beamai_jinja_expr. Keeping the two apart is what lets
%%% both be tested on their own.
%%% @end
%%%-------------------------------------------------------------------
-module(beamai_jinja_lexer).
...
-type token() :: beamai_jinja_expr_token().
...
-spec number(binary(), loc()) ->
          {ok, token(), binary(), loc()} | {error, loc(), beamai_jinja_reason()}.
...
-spec string_lit(binary(), loc()) ->
          {ok, binary(), binary(), loc()} | {error, loc(), beamai_jinja_reason()}.
