...
%%% beamai_html_engine. They are computed on the finished AST by warnings/1, which
%%% the plugin and the parse_transform call.
%%% @end
%%%-------------------------------------------------------------------
-module(beamai_jinja_parser).
...
-define(THROW(Loc, Reason), throw({beamai_jinja_parse, Loc, Reason})).

%%%===================================================================
%%% API
%%%===================================================================

-spec parse(unicode:chardata(), map()) -> {ok, nodes()} | beamai_jinja_error().
parse(Body, Opts) ->
    case beamai_jinja_scanner:scan(Body, Opts) of
        {error, _} = E -> E;
        {ok, Toks} ->
            try
                ...
                throw:{beamai_jinja_parse, {L, _C}, Reason} ->
                    {error, {beamai_html_text:source(Opts), L, Reason}}
            end
    end.
...
-spec body([beamai_jinja_token()], [atom()], atom(), loc()) ->
          {nodes(), [beamai_jinja_token()]}.
...
%% beamai_jinja_ast has something to match on (deviation J11).
...
%% too. beamai_jinja_ast has its own copy for the same reason beamai_mustache_ast does:
...
expr_all(Toks, _Loc) ->
    case beamai_jinja_expr:parse_all(Toks) of
        {ok, E}            -> E;
