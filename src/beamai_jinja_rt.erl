...
%%%
%%% Together with beamai_jinja_filters and beamai_jinja_tests this is the whole
%%% run-time dependency of generated code, and it depends on nothing but OTP
%%% and beamai_html_escape. No processes, no ets, no persistent_term, no process
%%% dictionary (architecture invariant 1).
%%%
%%% == Not a copy of beamai_mustache_rt ==
%%%
%%% `resolve/2' looks like `beamai_mustache_rt:lookup/2' and means something
...
escape({safe, D}) -> D;
escape(V)         -> beamai_html_escape:escape(to_binary(V), ?TAG, <<"&#34;">>).
...
apply_named(Kind, Name, Value, Extra) ->
    Params = case Kind of
                 filter -> maps:get(Name, beamai_jinja_filters:params(), []);
                 test   -> maps:get(Name, beamai_jinja_tests:params(), [])
             end,
    apply_named(Kind, Name, [Value, beamai_jinja_args:build(Params, Extra, [])]).

-spec apply_named(filter | test, atom(), [term()]) -> term().
apply_named(Kind, Name, Args) ->
    case beamai_jinja_ext:lookup(Kind, Name) of
