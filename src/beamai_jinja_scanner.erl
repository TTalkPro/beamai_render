...
-spec scan(unicode:chardata(), map()) ->
          {ok, [beamai_jinja_token()]} | beamai_jinja_error().
scan(Data, Opts) ->
    case beamai_html_text:template(Data) of
        {error, {invalid_utf8, _} = R} ->
            {error, {beamai_html_text:source(Opts), 1, R}};
        {ok, Bin0} ->
...
-spec chunks(binary(), loc(), [chunk()]) ->
          {ok, [chunk()]} | {error, loc(), beamai_jinja_reason()}.
chunks(Bin, Loc, Acc) ->
    ...
    case binary:match(Bin, [?BEAMAI_JINJA_EXPR_START, ?BEAMAI_JINJA_STMT_START,
                            ?BEAMAI_JINJA_COMM_START]) of
        nomatch ->
...
-spec tag(binary(), loc()) ->
          {ok, [chunk()], binary(), loc()} | {error, loc(), beamai_jinja_reason()}.
tag(<<"{#", Rest/binary>>, Loc) ->
    comment(Rest, Loc);
...
inline_tag(Bin, Loc, Kind, Stop) ->
    ...
    case beamai_jinja_lexer:tokens(Body, Loc0, Stop) of
        {error, _, _} = E ->
            E;
...
comment(Rest0, Loc) ->
    {LMark, Body, Loc0} = left_mark(Rest0, adv(Loc, 2)),
    case binary:match(Body, ?BEAMAI_JINJA_COMM_STOP) of
        nomatch ->
...
find_endraw(Bin, From) ->
    case binary:match(Bin, ?BEAMAI_JINJA_STMT_START, [{scope, {From, byte_size(Bin) - From}}]) of
        nomatch -> notfound;
        {Pos, 2} ->
    ...
    case word_at(Bin, P2) of
        {<<"endraw">>, P3} ->
            ...
            case binary:part(Bin, P5, min(2, byte_size(Bin) - P5)) of
                ?BEAMAI_JINJA_STMT_STOP -> {ok, P5 + 2, LMark, RMark};
                _                   -> no
            end;
        _ -> no
    end.
...
-spec weave([chunk()], map()) -> [beamai_jinja_token()].
weave(Chunks, Opts) ->
