...
in(V, A) -> beamai_jinja_rt:contains(maps:get(seq, A, []), V).

-spec eq(term(), map()) -> boolean().
eq(V, A) -> beamai_jinja_rt:eq(V, other(A)).
-spec ne(term(), map()) -> boolean().
ne(V, A) -> beamai_jinja_rt:ne(V, other(A)).
-spec lt(term(), map()) -> boolean().
lt(V, A) -> beamai_jinja_rt:lt(V, other(A)).
-spec le(term(), map()) -> boolean().
le(V, A) -> beamai_jinja_rt:le(V, other(A)).
-spec gt(term(), map()) -> boolean().
gt(V, A) -> beamai_jinja_rt:gt(V, other(A)).
-spec ge(term(), map()) -> boolean().
ge(V, A) -> beamai_jinja_rt:ge(V, other(A)).
...
registered(Kind, V) ->
    case name_of(V) of
        error -> false;
        Name  -> beamai_jinja_ext:lookup(Kind, Name) =/= error
    end.
