...
%%%
%%% greet(Name) ->                                   % (b)
%%%     beamai_jinja:inline(~"Hello {{ name }}!", #{name => Name}).
%%% '''
%%%
%%% <ul>
...
%%%   <li><b>(b) `beamai_jinja:inline/2'</b> with a binary literal is expanded in
...
%%%       {@link beamai_jinja_compiler:inline_forbidden/1}.</li>
...
collect_ext(A, Mod, St0) when is_atom(Mod) ->
    ...
    case beamai_jinja_ext:spec_module(Mod) of
        {error, Reason} ->
...
check_conflicts(A, Mod, Fs, Ts, St) ->
    Clashes = [{filter, N} || N <- Fs, beamai_jinja_ext:lookup(filter, N) =/= error]
        ++ [{test, N} || N <- Ts, beamai_jinja_ext:lookup(test, N) =/= error],
...
expr({call, A, {remote, _, {atom, _, beamai_jinja}, {atom, _, inline}},
      [Tpl, Ctx]} = Call, St) ->
...
do_expand(A, Bin, CtxExpr, Call, St) ->
    Opts = inline_opts(St),
    case beamai_jinja_parser:parse(Bin, Opts) of
...
build_inline(A, Nodes, CtxExpr, Call, Opts, St) ->
    ...
    case beamai_jinja_compiler:compile_inline(Nodes, Placeholder, Opts) of
        {error, {_F, TplLine, Reason}} ->
            {Call, error_at(shift(A, TplLine), Reason, St)};
        {ok, Expr0} ->
...
read_and_compile(Abs, Opts) ->
    case file:read_file(Abs) of
        ...
        {ok, Body} ->
            case beamai_jinja_parser:parse(Body, Opts) of
                {error, E}  -> {error, E};
                {ok, Nodes} ->
                    case beamai_jinja_compiler:forms(Nodes, Opts) of
                        {error, E}        -> {error, E};
...
%% beamai_jinja_compiler:forms/2 builds a whole module, which is the wrong shape
...
%% beamai_jinja_rt, another generated module -- must not be touched.
...
format_error(inline_not_literal) ->
    "beamai_jinja:inline/2 was left as a run-time call because its first argument "
...
format_error({filter_name_conflict, Name, Mod}) ->
    beamai_jinja_ext:format_error({filter_name_conflict, Name, Mod});
