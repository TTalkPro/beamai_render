...
render_string(Template, Ctx, Opts) ->
    case compile_set(Template, Opts) of
        ...
        {error, _} = E -> erlang:error({beamai_mustache, E})
    end.

%% @doc Render an inline template.
%%
%% With the beamai_mustache_transform parse_transform applied and a literal first
...
inline(Template, Ctx) ->
    case beamai_mustache_parser:parse(Template, #{source => <<"inline">>}) of
        {error, _} = E ->
            erlang:error({beamai_mustache, E});
        {ok, Ast} ->
            case has_partial(Ast) of
                true ->
                    erlang:error({beamai_mustache,
                                  {error, {<<"inline">>, 1,
...
-spec has_partial([beamai_mustache_node()]) -> boolean().
has_partial([])                            -> false;
...
-spec compile_set(binary(), map()) -> {ok, module()} | beamai_mustache_error().
...
-spec compile_all(binary(), map(), binary(), module(), map()) ->
          {ok, module()} | beamai_mustache_error().
compile_all(Template, Partials, Prefix, Main, Opts) ->
    %% origin => string keeps beamai_mustache_dev from treating these modules,
    ...
    Units = [{Main, <<"main">>, Template}
             | [{beamai_mustache_ast:module_name(N, Base), N, T}
...
-spec parse_units([{module(), binary(), binary()}], map(),
                  [{module(), binary(), [beamai_mustache_node()]}]) ->
          {ok, [{module(), binary(), [beamai_mustache_node()]}]} | beamai_mustache_error().
parse_units([], _Base, Acc) ->
    {ok, lists:reverse(Acc)};
parse_units([{Mod, Name, Text} | Rest], Base, Acc) ->
    Opts = Base#{module => Mod, source => <<Name/binary, ".mustache">>},
    case beamai_mustache_parser:parse(Text, Opts) of
        {error, _} = E -> E;
...
-spec stub_missing([{module(), binary(), [beamai_mustache_node()]}], map()) -> ok.
...
-spec refs([beamai_mustache_node()], map()) -> [module()].
refs(Nodes, Base) -> refs(Nodes, Base, []).

refs([], _Base, Acc) ->
    Acc;
refs([{partial, _, Name, _} | Rest], Base, Acc) when is_binary(Name) ->
    refs(Rest, Base, [beamai_mustache_ast:module_name(Name, Base) | Acc]);
...
-spec build([{module(), binary(), [beamai_mustache_node()]}], map()) ->
          ok | beamai_mustache_error().
build([], _Base) ->
    ok;
build([{Mod, Name, Ast} | Rest], Base) ->
    Opts = Base#{module => Mod, source => <<Name/binary, ".mustache">>},
    case beamai_mustache_compiler:forms(Ast, Opts) of
        {error, _} = E -> E;
        {ok, Forms, _Deps} ->
...
-spec load(module(), [erl_parse:abstract_form()], map()) -> ok | beamai_mustache_error().
load(Mod, Forms, Opts) ->
