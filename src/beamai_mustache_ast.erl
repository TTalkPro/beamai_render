...
-type nodes() :: [beamai_mustache_node()].
...
-spec postprocess(nodes(), map()) ->
          {ok, nodes(), [module()]} | beamai_mustache_error().
...
-spec map_body(fun((nodes()) -> nodes()), beamai_mustache_node()) -> beamai_mustache_node().
...
-spec merge(nodes(), [{beamai_mustache_loc(), binary()}], nodes()) -> nodes().
merge([], Pending, Acc) ->
...
-spec flush([{beamai_mustache_loc(), binary()}], nodes()) -> nodes().
flush([], Acc) ->
    Acc;
flush(Pending, Acc) ->
...
resolve([{partial, Loc, Name, Indent} | Rest], Opts, Deps, Acc) when is_binary(Name) ->
    case exists(Name, Opts) of
        false ->
            {error, {beamai_html_text:source(Opts), element(1, Loc),
...
-spec body_of(beamai_mustache_node()) -> nodes() | none.
