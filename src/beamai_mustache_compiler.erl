...
%%% @end
%%%-------------------------------------------------------------------
-module(beamai_mustache_compiler).
...
%% The form constructors live in beamai_html_forms, shared with the other engines
...
-spec forms([beamai_mustache_node()], map()) ->
          {ok, [erl_parse:abstract_form()], [module()]} | beamai_mustache_error().
forms(Nodes0, Opts) ->
    case beamai_mustache_ast:postprocess(Nodes0, Opts) of
        {error, _} = E -> E;
        {ok, Nodes, Deps} ->
            St0 = new_state(Opts),
            try body_exprs(Nodes, true, St0) of
                ...
                throw:{beamai_mustache_error, Err} -> Err
            end
    end.

%% @doc Compile a template into a single expression for inline expansion.
%%
%% Used by the parse_transform when it rewrites beamai_mustache:inline/2 with a
...
-spec compile_inline([beamai_mustache_node()], erl_parse:abstract_expr(), map()) ->
          {ok, erl_parse:abstract_expr()} | beamai_mustache_error().
compile_inline(Nodes0, CtxExpr, Opts) ->
    case beamai_mustache_ast:merge_text(beamai_mustache_ast:drop_empty(Nodes0)) of
        Nodes ->
...
new_state(Opts0) ->
    Opts = beamai_html_text:opts(Opts0),
    #cs{opts     = Opts,
        file     = beamai_html_text:source(Opts),
...
module_forms(Exprs, Deps, St) ->
    ...
    Allowed = [erlang, beamai_mustache_rt] ++ Deps ++ St#cs.ext_remotes,
...
static_text(Exprs) -> beamai_html_forms:static_text(Exprs, ?IV).
...
source_attr(#cs{opts = Opts}) ->
    Attr = #{path  => path_of(Opts),
             ...
             vsn   => ?BEAMAI_MUSTACHE_VSN,
             opts  => normalize_opts(Opts)},
    %% Only recorded when it is not the default, so a file template's
    %% generated .erl is unchanged. See beamai_mustache_source() in the header.
    case maps:get(origin, Opts, file) of
        file   -> Attr;
        Origin -> Attr#{origin => Origin}
    end.

%% Generated modules may only call beamai_mustache_rt, other generated modules and
...
%% assume.
-spec body_exprs([beamai_mustache_node()], boolean(), state()) ->
          {[erl_parse:abstract_expr()], state()}.
body_exprs(Nodes, Bol, St) ->
    body_loop(Nodes, St#cs{bol = Bol}, []).

-spec body_loop([beamai_mustache_node()], state(), [erl_parse:abstract_expr()]) ->
          {[erl_parse:abstract_expr()], state()}.
body_loop([], St, Acc) ->
...
-spec node_exprs(beamai_mustache_node(), state()) ->
          {[erl_parse:abstract_expr()], state()}.
    ...
    case maps:get(Marker, St#cs.exts, undefined) of
        ...
        Mod ->
            ...
            try Mod:compile_tag(Marker, Keys, BodyExprs, TagOpts) of
                ...
                C:R -> throw({beamai_mustache_error,
                              {error, {St#cs.file, L,
...
-spec aux(atom(), beamai_mustache_loc(), beamai_mustache_keys(), [beamai_mustache_node()],
...
%% beamai_mustache_dev each rolled their own the two would drift, and dev would
...
source_hash(Body, Opts) ->
    erlang:md5(term_to_binary({Body, normalize_opts(Opts), ?BEAMAI_MUSTACHE_VSN},
                              [deterministic])).

%% Only the options that can change the generated code, as a sorted list so
%% the result does not depend on map iteration order.
%%
%% Values go through beamai_html_text:opts/1 first, so `views' as a string and
...
%% plugin and beamai_mustache_dev would disagree about staleness the moment one of
...
normalize_opts(Opts0) ->
    Opts = beamai_html_text:opts(as_map(Opts0)),
    lists:sort(
...
keys(L, Keys)  -> beamai_html_forms:atoms(L, Keys).
rt(L, F, Args) -> rem_call(L, beamai_mustache_rt, F, Args).
hd_stack(L)    -> beamai_html_forms:hd_call(L, v(L, ?SV)).
...
path_of(Opts) -> beamai_html_text:source(Opts).
