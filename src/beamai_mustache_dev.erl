...
-spec source(module()) -> {ok, beamai_mustache_source()} | {error, not_a_template_module}.
source(Mod) ->
...
%% beamai_mustache_compiler:source_hash/2 the plugin uses. Two independent
...
stale(Mod) ->
    case source(Mod) of
        ...
        {ok, #{path := Path, stamp := Stamp, vsn := Vsn} = Src} ->
            case Vsn =/= ?BEAMAI_MUSTACHE_VSN of
...
-spec stale_by_stamp(binary() | string(), binary(), beamai_mustache_source()) ->
          boolean() | {error, term()}.
stale_by_stamp(Path, Stamp, #{opts := Opts}) ->
    case file:read_file(Path) of
        {error, R} -> {error, {R, Path}};
        {ok, Body} -> beamai_mustache_compiler:source_hash(Body, Opts) =/= Stamp
    end.
