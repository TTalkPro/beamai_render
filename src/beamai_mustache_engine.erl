%%%-------------------------------------------------------------------
%%% @doc beamai_html_engine adapter for the mustache engine.
%%%
...
%%% @end
%%%-------------------------------------------------------------------
-module(beamai_mustache_engine).

-behaviour(beamai_html_engine).
...
parse(Body, Opts) -> beamai_mustache_parser:parse(Body, Opts).

-spec forms(term(), map()) ->
          {ok, [erl_parse:abstract_form()], [module()]} | {error, term()}.
forms(Ast, Opts) -> beamai_mustache_compiler:forms(Ast, Opts).

-spec source_hash(binary(), map() | [{atom(), term()}]) -> binary().
source_hash(Body, Opts) -> beamai_mustache_compiler:source_hash(Body, Opts).

-spec module_name(binary(), map()) -> module().
module_name(Name, Opts) -> beamai_mustache_ast:module_name(Name, Opts).
...
default_prefix() -> beamai_mustache_ast:default_prefix().

-spec config_key() -> atom().
config_key() -> mustache_opts.

-spec known_keys() -> [atom()].
known_keys() -> ?BEAMAI_HTML_MUSTACHE_KEYS.
