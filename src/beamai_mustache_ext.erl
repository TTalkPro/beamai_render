...
%%% `beamai_mustache:inline/2'. `compile_tag/4' produces an abstract expression,
%%% which only means something at compile time. An inline template that uses a
%%% custom marker therefore requires
%%% `-compile({parse_transform, beamai_mustache_transform})'; without it the call
...
%% @doc The markers beamai_render claims for itself: `# ^ / > ! = & { } + - *'.
%%
%% The set is defined once, in include/beamai_mustache.hrl. Everything that needs
...
builtin_markers() -> ?BEAMAI_MUSTACHE_BUILTIN_MARKERS.

-spec is_reserved(char()) -> boolean().
is_reserved(C) -> lists:member(C, ?BEAMAI_MUSTACHE_BUILTIN_MARKERS).
...
format_error({not_an_ext_module, Mod}) ->
    io_lib:format(
      ...
      "-behaviour(beamai_mustache_ext).", [Mod]);
...
format_error({marker_reserved, C, Mod}) ->
    io_lib:format(
      "marker ~ts (claimed by ~w) is reserved by beamai_render. The builtin markers "
...
-spec rt_call(erl_anno:anno(), atom(), [erl_parse:abstract_expr()]) ->
          erl_parse:abstract_expr().
rt_call(L, F, Args) ->
    {call, L, {remote, L, {atom, L, beamai_mustache_rt}, {atom, L, F}}, Args}.
