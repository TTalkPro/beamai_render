...
%%% are separate passes in beamai_mustache_ast (T09) so each can be tested alone.
%%%
%%% Derived in part from bbmustache (MIT, Copyright (c) 2015 Hinagiku Soranoba).
%%% @end
%%%-------------------------------------------------------------------
-module(beamai_mustache_parser).
...
-type open() :: toplevel | {char(), beamai_mustache_keys(), beamai_mustache_loc()}.

%%%===================================================================
%%% API
%%%===================================================================

-spec parse(unicode:chardata() | [beamai_mustache_token()]) ->
          {ok, [beamai_mustache_node()]} | beamai_mustache_error().
parse(Input) -> parse(Input, #{}).

-spec parse(unicode:chardata() | [beamai_mustache_token()], map()) ->
          {ok, [beamai_mustache_node()]} | beamai_mustache_error().
...
parse([], Opts) ->
    do_parse([], Opts);
parse(Data, Opts) ->
    case beamai_mustache_scanner:scan(Data, Opts) of
        {error, _} = E -> E;
        {ok, Tokens}   -> do_parse(Tokens, Opts)
    end.

-spec do_parse([beamai_mustache_token()], map()) ->
          {ok, [beamai_mustache_node()]} | beamai_mustache_error().
do_parse(Tokens, Opts0) ->
    Opts = beamai_html_text:opts(Opts0),
    St = #st{file = beamai_html_text:source(Opts), ext = ext_table(Opts)},
...
-spec nodes([beamai_mustache_token()], open(), [beamai_mustache_node()], #st{}) ->
          {ok, [beamai_mustache_node()], [beamai_mustache_token()]} | beamai_mustache_error().
...
-spec block(char(), beamai_mustache_loc(), beamai_mustache_keys(), [beamai_mustache_token()],
            open(), [beamai_mustache_node()], #st{}) ->
          {ok, [beamai_mustache_node()], [beamai_mustache_token()]} | beamai_mustache_error().
...
-spec ext_block(char(), beamai_mustache_loc(), beamai_mustache_keys(), [beamai_mustache_token()],
                open(), [beamai_mustache_node()], #st{}) ->
          {ok, [beamai_mustache_node()], [beamai_mustache_token()]} | beamai_mustache_error().
...
-spec node_of(char(), beamai_mustache_loc(), beamai_mustache_keys(), [beamai_mustache_node()]) ->
...
%% beamai_mustache_rt:lookup/2 recognises as the implicit iterator and answers
...
-spec keys(binary()) -> beamai_mustache_keys().
keys(Bin0) ->
    Bin = << <<X:8>> || <<X:8>> <= Bin0,
                        X =/= $\s, X =/= $\t, X =/= $\r, X =/= $\n >>,
    case Bin =:= <<>> orelse Bin =:= <<".">> of
        true  -> [?BEAMAI_MUSTACHE_DOT];
...
-spec partial_name(binary()) -> {ok, binary()} | {error, beamai_mustache_reason()}.
...
-spec line(beamai_mustache_loc()) -> pos_integer().
line({L, _C}) -> L.

-spec line_of(beamai_mustache_token()) -> pos_integer().
