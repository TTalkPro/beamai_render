...
%%%
%%%   * `to_binary/1' raises `{beamai_mustache, {not_renderable, Term}}' for
...
%%
%% `[?BEAMAI_MUSTACHE_DOT]' is the implicit iterator `{{.}}' and yields the top
...
-spec lookup(beamai_mustache_keys(), Stack :: [term()]) -> term().
lookup([?BEAMAI_MUSTACHE_DOT], [Top | _]) -> Top;
lookup([?BEAMAI_MUSTACHE_DOT], [])        -> undefined;
lookup([K | Rest], Stack) ->
...
-spec find_frame(beamai_mustache_key(), [term()]) -> {ok, term()} | error.
find_frame(_K, []) -> error;
...
-spec descend(beamai_mustache_keys(), term()) -> term().
descend([], V) -> V;
...
%% Both live in beamai_html_escape, shared with the other engines
...
%%
%% Generated modules call beamai_mustache_rt:escape/1 and beamai_mustache_rt:to_binary/1
%% by name, so these exports can never be removed.
-spec to_binary(term()) -> binary().
to_binary(V) -> beamai_html_escape:to_binary(V, beamai_mustache).

-spec escape(term()) -> iodata().
escape(V) -> beamai_html_escape:escape(V, beamai_mustache).
...
section(F, _Body, _Stack, _Indent, _D) when is_function(F, 1) ->
    error({beamai_mustache, {lambda_depth_exceeded, ?MAX_LAMBDA_DEPTH}});
