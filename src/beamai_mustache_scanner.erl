...
%%% @end
%%%-------------------------------------------------------------------
-module(beamai_mustache_scanner).

-include("beamai_mustache.hrl").

-export([scan/1, scan/2]).

-record(st, {start         = ?BEAMAI_MUSTACHE_START :: binary(),
             stop          = ?BEAMAI_MUSTACHE_STOP  :: binary(),
             file          = <<"nofile">>       :: binary(),
             markers       = []                 :: [char()],
             block_markers = []                 :: [char()]}).
...
-type skip_token() :: {skip, beamai_mustache_loc(), char()}.
-type wtoken()     :: beamai_mustache_token() | skip_token().

%%%===================================================================
%%% API
%%%===================================================================

-spec scan(unicode:chardata()) -> {ok, [beamai_mustache_token()]} | beamai_mustache_error().
scan(Data) -> scan(Data, #{}).
...
-spec scan(unicode:chardata(), map()) ->
          {ok, [beamai_mustache_token()]} | beamai_mustache_error().
scan(Data, Opts0) ->
...
-spec do_scan(binary(), #st{}, pos_integer(), pos_integer(), [wtoken()]) ->
          {ok, [wtoken()]} | beamai_mustache_error().
...
-spec marker_of(binary(), #st{}) -> {beamai_mustache_marker(), binary()} | {error, term()}.
...
-spec emit(beamai_mustache_marker(), binary(), binary(), non_neg_integer(), binary(),
...
-spec drop_skips([wtoken()]) -> [beamai_mustache_token()].
