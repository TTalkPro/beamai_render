...
%%%
%%% greet(Name) ->                                       % (b)
%%%     beamai_mustache:inline(~"Hello {{name}}!", #{name => Name}).
%%% '''
%%%
%%% <ul>
...
%%%       beam.</li>
%%%   <li><b>(b) `beamai_mustache:inline/2'</b> with a binary literal is expanded
...
%%% {@link beamai_html_path:scan/1}) and touching the `.erl' when the
...
%%% @end
%%%-------------------------------------------------------------------
-module(beamai_mustache_transform).
...
parse_transform(Forms, Opts) ->
    ...
    %% pass 2: expand beamai_mustache:inline/2, drop consumed attributes
...
add_tag(A, Mod, Which, St0) ->
    ...
    case pre_check_marker(A, Mod, Which, St1) of
        {error, St2} -> St2;
        ok ->
            case beamai_mustache_ext:describe(Mod, vopts(St1)) of
...
pre_check_marker(A, Mod, {one, C, _Fun}, St) ->
    case {beamai_mustache_ext:is_valid_marker(C), beamai_mustache_ext:is_reserved(C)} of
...
expr({call, A, {remote, _, {atom, _, beamai_mustache}, {atom, _, inline}},
      [Tpl, Ctx]} = Call, St) ->
...
do_expand(A, Bin, CtxExpr, Call, St) ->
    Opts = inline_opts(St),
    case beamai_mustache_parser:parse(Bin, Opts) of
...
build_inline(A, Nodes, CtxExpr, Call, Opts, St) ->
    ...
    case beamai_mustache_compiler:compile_inline(Nodes, Placeholder, Opts) of
        {error, {_F, TplLine, Reason}} ->
            {Call, error_at(shift(A, TplLine), Reason, St)};
        {ok, Expr0} ->
...
%% beamai_mustache_compiler always names the stack `S' and the indent `I'. Two
...
read_and_compile(Abs, Opts) ->
    case file:read_file(Abs) of
        ...
        {ok, Body} ->
            case beamai_mustache_parser:parse(Body, Opts) of
                {error, E}     -> {error, E};
                {ok, Nodes}    ->
                    case beamai_mustache_compiler:forms(Nodes, Opts) of
                        {error, E}          -> {error, E};
...
%% beamai_mustache_compiler:forms/2 builds a whole module, which is the wrong shape
...
%% must not: `beamai_mustache_rt:lookup/2' is not one of ours to rename.
...
-spec vopts(#st{}) -> beamai_mustache_ext:vopts().
vopts(St) ->
...
%%
%% Extension problems are handed to beamai_mustache_ext so that the message a user
...
format_error(inline_not_literal) ->
    "the first argument of beamai_mustache:inline/2 is not a binary literal, so it "
...
format_error({template_partial_needs_plugin, Name, Deps}) ->
    io_lib:format(
      ...
      "Those modules are produced by the rebar3 beamai_render plugin; without it (or a "
...
format_error(Reason) ->
    beamai_mustache_ext:format_error(Reason).
