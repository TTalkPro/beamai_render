...
%%%-------------------------------------------------------------------
-module(beamai_html_forms_tests).

-include_lib("eunit/include/eunit.hrl").

-define(L, (beamai_html_forms:anno(0))).
...
bin_round_trips_through_source_test() ->
    [?assertEqual(Bin, eval_source(beamai_html_forms:bin(?L, Bin)))
...
concat_bin_test() ->
    {bin, _, [Head | _]} = beamai_html_forms:concat_bin(?L, 'I', <<"  ">>),
    ?assertEqual({bin_element, ?L, {var, ?L, 'I'}, default, [binary]}, Head).

mklist_test() ->
    ?assertEqual({nil, ?L}, beamai_html_forms:mklist(?L, [])),
    ?assertEqual({cons, ?L, {atom, ?L, a}, {nil, ?L}},
                 beamai_html_forms:mklist(?L, [{atom, ?L, a}])).

atoms_test() ->
    ?assertEqual([a, b], eval_source(beamai_html_forms:atoms(?L, [a, b]))).

abstract_carries_the_anno_test() ->
    E = beamai_html_forms:abstract(beamai_html_forms:anno(7), #{a => [1, <<"x">>]}),
    ...
    ?assert(lists:all(fun(A) -> A =:= beamai_html_forms:anno(7) end, annos(E))).
...
param_underscores_unused_test() ->
    Used   = {var, ?L, 'S'},
    Unused = {atom, ?L, ok},
    ?assertEqual({var, ?L, 'S'},  beamai_html_forms:param(?L, 'S', Used)),
    ?assertEqual({var, ?L, '_S'}, beamai_html_forms:param(?L, 'S', Unused)).

uses_var_descends_test() ->
    Deep = {call, ?L, {atom, ?L, f}, [{cons, ?L, {var, ?L, 'X'}, {nil, ?L}}]},
    ?assert(beamai_html_forms:uses_var(Deep, 'X')),
    ?assertNot(beamai_html_forms:uses_var(Deep, 'Y')).
...
remotes_test() ->
    E = beamai_html_forms:rem_call(?L, mod_a, f,
                               [beamai_html_forms:rem_call(?L, mod_b, g, [])]),
    ?assertEqual([mod_a, mod_b], lists:sort(beamai_html_forms:remotes(E, []))).

remotes_ignores_local_calls_test() ->
    ?assertEqual([], beamai_html_forms:remotes(beamai_html_forms:loc_call(?L, f, []), [])).
...
literal_of_test() ->
    ?assertEqual(<<>>,       beamai_html_forms:literal_of(beamai_html_forms:bin(?L, <<>>))),
    ?assertEqual(<<"hi">>,   beamai_html_forms:literal_of(beamai_html_forms:bin(?L, <<"hi">>))),
    ?assertEqual(<<"中"/utf8>>,
                 beamai_html_forms:literal_of(beamai_html_forms:bin(?L, <<"中"/utf8>>))),
    ?assertEqual(<<255>>,    beamai_html_forms:literal_of(beamai_html_forms:bin(?L, <<255>>))),
    ?assertEqual(error,      beamai_html_forms:literal_of({var, ?L, 'X'})).

static_text_folds_literals_and_the_indent_var_test() ->
    Exprs = [beamai_html_forms:bin(?L, <<"a">>),
             {var, ?L, 'I'},
             beamai_html_forms:bin(?L, <<"b">>)],
    ?assertEqual({yes, <<"ab">>}, beamai_html_forms:static_text(Exprs, 'I')).

static_text_refuses_anything_dynamic_test() ->
    Exprs = [beamai_html_forms:bin(?L, <<"a">>), {var, ?L, 'X'}],
    ?assertEqual(no, beamai_html_forms:static_text(Exprs, 'I')).
...
depends_only_on_otp_test() ->
    {ok, {_, [{imports, Imports}]}} =
        beam_lib:chunks(code:which(beamai_html_forms), [imports]),
