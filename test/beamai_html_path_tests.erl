...
template_spec_test() ->
    ?assertEqual({ok, {index, "views/index.mustache"}},
                 beamai_html_path:template_spec({index, "views/index.mustache"})),
    %% Bare path: the name comes from the basename.
    ?assertEqual({ok, {index, "views/index.j2"}},
                 beamai_html_path:template_spec("views/index.j2")),
    ?assertEqual({ok, {my_page, "a/My-Page.j2"}},
                 beamai_html_path:template_spec("a/My-Page.j2")),
    ?assertEqual({ok, {index, "views/index.j2"}},
                 beamai_html_path:template_spec(<<"views/index.j2">>)),
    ?assertEqual(error, beamai_html_path:template_spec("")),
             ...
             [{"only the mustache attributes",
              ...
              {"only the jinja attributes, both shapes",
               ?_assertEqual({ok, [{b, "v/b.j2"}, {c, "v/c.j2"}]},
                             beamai_html_path:scan(File, jinja_template))},
              {"an attribute nobody declared",
               ?_assertEqual({ok, []}, beamai_html_path:scan(File, other_template))},
              {"a file that is not there",
               ?_assertMatch({error, _},
                             beamai_html_path:scan(filename:join(Dir, "no.erl"),
                                               jinja_template))}]
     end}.
    ...
    {setup, fun setup/0, fun cleanup/1,
     fun(Dir) ->
             ...
             ok = file:write_file(File,
                                  ...
                                  "-mustache_template({a, \"v/a.mustache\"}).\n"),
             [?_assertEqual({ok, [{a, "v/a.mustache"}]},
                            beamai_mustache_path:scan(File)),
              ?_assertEqual({ok, {index, "views/index.mustache"}},
                            beamai_mustache_path:template_spec("views/index.mustache"))]
     end}.

text_shell_forwards_test() ->
    ?assertEqual({ok, <<"x">>}, beamai_mustache_text:template("x")),
    ?assertEqual(<<"x">>, beamai_mustache_text:path("x")),
    ?assertEqual(<<"nofile">>, beamai_mustache_text:source(#{})),
    ?assertEqual(#{source => <<"a">>}, beamai_mustache_text:opts(#{source => "a"})),
    ?assertEqual("x", beamai_mustache_text:to_list(<<"x">>)).

%%%===================================================================

setup() ->
    Dir = filename:join(["/tmp", "beamai_render_path_tests",
