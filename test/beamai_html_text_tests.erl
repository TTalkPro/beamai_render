...
%%%
%%% The point of beamai_html_text is that no module downstream has to ask what
...
template_accepts_binary_test() ->
    ?assertEqual({ok, <<"a{{x}}b">>}, beamai_html_text:template(<<"a{{x}}b">>)).

template_accepts_string_test() ->
    ?assertEqual({ok, <<"abc">>}, beamai_html_text:template("abc")).

template_accepts_iolist_test() ->
    ?assertEqual({ok, <<"abc">>}, beamai_html_text:template([<<"a">>, "b", [<<"c">>]])).

template_keeps_utf8_test() ->
    ?assertEqual({ok, ?CJK}, beamai_html_text:template(?CJK)).
...
template_rejects_invalid_utf8_test() ->
    ?assertEqual({error, {invalid_utf8, 3}}, beamai_html_text:template(?LATIN1)).

template_reports_offset_test() ->
    ?assertMatch({error, {invalid_utf8, 0}},
                 beamai_html_text:template(<<255, 254>>)).
...
path_normalises_test_() ->
    [?_assertEqual(<<"views">>, beamai_html_text:path("views")),
     ?_assertEqual(<<"views">>, beamai_html_text:path(<<"views">>)),
     ?_assertEqual(?CJK, beamai_html_text:path(?CJK)),
     ?_assertEqual(?LATIN1, beamai_html_text:path(?LATIN1))].
...
opts_leaves_absent_keys_absent_test() ->
    ?assertEqual(#{module => m}, beamai_html_text:opts(#{module => m})).

source_defaults_test_() ->
    [?_assertEqual(<<"nofile">>, beamai_html_text:source(#{})),
     ?_assertEqual(<<"x.mustache">>, beamai_html_text:source(#{source => "x.mustache"}))].

to_list_round_trip_test_() ->
    [?_assertEqual("abc", beamai_html_text:to_list(<<"abc">>)),
     ?_assertEqual(?CJK, unicode:characters_to_binary(
                           beamai_html_text:to_list(?CJK), utf8))].
...
string_and_binary_agree_test() ->
    Opts = #{source => <<"t">>},
    {ok, A} = beamai_mustache_parser:parse(<<"a{{x}}b">>, Opts),
    {ok, B} = beamai_mustache_parser:parse("a{{x}}b", Opts),
    ?assertEqual(A, B).

module_name_accepts_string_prefix_test() ->
    %% beamai_mustache_ast:module_name/2 concatenates binaries, so a string prefix
    %% used to crash. opts/1 is what makes it work.
    Opts = beamai_html_text:opts(#{prefix => "view_"}),
    ?assertEqual(view_shared_item,
                 beamai_mustache_ast:module_name(<<"shared/item">>, Opts)).

%% The build stamp must not depend on how a path was spelled, or the plugin
%% and beamai_mustache_dev would disagree about staleness the moment one of them
%% passed a string.
stamp_is_representation_independent_test() ->
    Body = <<"{{x}}">>,
    A = beamai_mustache_compiler:source_hash(Body, #{views => "views", prefix => "view_"}),
    B = beamai_mustache_compiler:source_hash(Body, #{views => <<"views">>,
                                                 prefix => <<"view_">>}),
    ?assertEqual(A, B).

stamp_still_changes_with_options_test() ->
    Body = <<"{{x}}">>,
    A = beamai_mustache_compiler:source_hash(Body, #{prefix => <<"view_">>}),
    B = beamai_mustache_compiler:source_hash(Body, #{prefix => <<"tpl_">>}),
    ?assertNotEqual(A, B).
...
invalid_utf8_template_is_a_normal_error_test() ->
    ?assertEqual({error, {<<"views/bad.mustache">>, 1, {invalid_utf8, 3}}},
                 beamai_mustache_parser:parse(?LATIN1, #{source => "views/bad.mustache"})).

utf8_template_renders_intact_test() ->
    Tpl = <<"[", (?CJK)/binary, "{{x}}]">>,
    ?assertEqual(<<"[", (?CJK)/binary, "v]">>,
                 beamai_mustache:render_string(Tpl, #{x => <<"v">>})).
