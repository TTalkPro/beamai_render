...
%%% build failure.
%%%-------------------------------------------------------------------
-module(beamai_jinja_ast_tests).

-include_lib("eunit/include/eunit.hrl").

parse(Bin) -> {ok, N} = beamai_jinja_parser:parse(Bin, #{source => <<"t">>}), N.
fold(Bin)  -> beamai_jinja_ast:fold_constants(parse(Bin)).
merge(Bin) -> beamai_jinja_ast:merge_text(parse(Bin)).
...
merge_descends_into_every_body_test() ->
    ...
    [?assert(has_merged_text(beamai_jinja_ast:merge_text(parse(C)))) || C <- Cases].

has_merged_text(Nodes) ->
    lists:any(fun({text, _, <<"ab">>}) -> true;
                 (N) -> lists:any(fun has_merged_text/1, beamai_jinja_ast:bodies(N))
              end, Nodes).
...
%% Folding evaluates through beamai_jinja_rt, so `{{ 1 + 2 }}' and `{{ a + b }}'
...
postprocess_merges_after_folding_test() ->
    {ok, Nodes, []} = beamai_jinja_ast:postprocess(parse(<<"a{% if false %}x{% endif %}b">>),
                                               #{source => <<"t">>}),
...
module_names_test() ->
    ?assertEqual(j2_index, beamai_jinja_ast:module_name(<<"index.j2">>, #{})),
    ?assertEqual(j2_layout_base, beamai_jinja_ast:module_name(<<"layout/base.j2">>, #{})),
    ?assertEqual(view_a_b, beamai_jinja_ast:module_name(<<"a-b.j2">>,
                                                    #{prefix => <<"view_">>})),
    %% The suffix is stripped only when it is the configured one.
    ?assertEqual(j2_index_html, beamai_jinja_ast:module_name(<<"index.html">>, #{})).

targets_resolve_to_modules_test() ->
    Opts = #{source => <<"t">>, templates => #{<<"b.j2">> => <<"x">>}},
    {ok, Nodes, Deps} =
        beamai_jinja_ast:postprocess(parse(<<"{% include \"b.j2\" %}">>), Opts),
...
dynamic_target_test() ->
    ...
    ?assertMatch({error, {_, _, {dynamic_target_unsupported, _}}},
                 beamai_jinja_ast:postprocess(parse(<<"{% include name %}">>), Opts)).
...
self_extends_is_a_cycle_test() ->
    ...
    ?assertMatch({error, {_, _, {extends_cycle, [j2_b]}}},
                 beamai_jinja_ast:postprocess(parse(<<"{% extends \"b.j2\" %}">>), Opts)).
...
merge_is_linear_test() ->
    ...
    {Micros, [{text, _, Merged}]} =
        timer:tc(fun() -> beamai_jinja_ast:merge_text(Nodes) end),
