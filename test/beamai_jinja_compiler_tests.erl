...
%%%-------------------------------------------------------------------
-module(beamai_jinja_compiler_tests).
...
compile(Src, Extra) ->
    ...
    {ok, Nodes} = beamai_jinja_parser:parse(Src, Opts),
    {ok, Forms, Deps} = beamai_jinja_compiler:forms(Nodes, Opts),
    {Mod, Forms, Deps}.

load(Src) -> load(Src, #{}).

%% Loaded probes are marked as string-compiled, which is what they are: they
%% have no file on disk, and without the mark beamai_jinja_dev:check/0 would report
...
%% beamai_jinja_dev:check/0 would rightly complain about it.
...
only_the_direct_parent_is_named_test() ->
    ...
    Called = lists:usort(beamai_html_forms:remotes(Forms, [])),
...
generated_code_calls_only_the_allowed_set_test() ->
    ...
    Allowed = [erlang, lists, maps, code, beamai_jinja_rt, beamai_jinja_filters,
               beamai_jinja_tests, beamai_html_escape, Mod | Deps],
    ?assertEqual([], [M || M <- lists:usort(beamai_html_forms:remotes(Forms, [])),
...
check_remotes_rejects_an_unknown_module_test() ->
    %% beamai_jinja_ext_tests_ext is only allowed because the compiler collects it
    ...
    ?assertMatch({error, {_, _, {unknown_filter, money}}},
                 begin
                     ...
                     {ok, N} = beamai_jinja_parser:parse(<<"{{ 1|money('E') }}">>, Opts),
                     beamai_jinja_compiler:forms(N, Opts)
                 end).
...
stamp_covers_every_shaping_option_test() ->
    Base = #{},
    S0 = beamai_jinja_compiler:source_hash(<<"x">>, Base),
    [?assertNotEqual(S0, beamai_jinja_compiler:source_hash(<<"x">>, Base#{K => V}))
...
stamp_ignores_views_abs_and_suffix_test() ->
    S = beamai_jinja_compiler:source_hash(<<"x">>, #{}),
    ?assertEqual(S, beamai_jinja_compiler:source_hash(<<"x">>,
                                                  #{views_abs => <<"/home/a">>})),
    ?assertEqual(S, beamai_jinja_compiler:source_hash(<<"x">>,
                                                  #{views_abs => <<"/home/b">>})),
    ?assertEqual(S, beamai_jinja_compiler:source_hash(<<"x">>, #{suffix => <<".x">>})).

%% Whether the stamp matched must not depend on how a caller spelled a path.
stamp_normalises_paths_test() ->
    ?assertEqual(beamai_jinja_compiler:source_hash(<<"x">>, #{views => "views"}),
                 beamai_jinja_compiler:source_hash(<<"x">>, #{views => <<"views">>})).

stamp_follows_the_content_test() ->
    ?assertNotEqual(beamai_jinja_compiler:source_hash(<<"a">>, #{}),
                    beamai_jinja_compiler:source_hash(<<"b">>, #{})).
...
filters_are_statically_bound_test() ->
    ...
    ?assert(lists:member(beamai_jinja_filters, beamai_html_forms:remotes(Forms, []))),
...
required_block_without_an_implementation_test() ->
    ...
    ?assertError({beamai_jinja, {required_block_not_provided, m}}, Mod:render(#{})).
...
missing_imported_macro_test() ->
    Opts = #{module => j2_probe_i, source => <<"t">>,
             templates => #{<<"l.j2">> => <<>>},
             macros => #{j2_l => [known]}},
    {ok, N} = beamai_jinja_parser:parse(<<"{% from \"l.j2\" import nope %}">>, Opts),
