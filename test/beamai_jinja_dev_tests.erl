...
setup() ->
    Dir = filename:join(["/tmp", "beamai_render_jinja_dev_" ++ integer_to_list(erlang:unique_integer([positive]))]),
...
build(Path, Body) ->
    ...
    Opts = Opts0#{source => list_to_binary(Path),
                  stamp  => beamai_jinja_compiler:source_hash(Body, Opts0),
                  mtime  => mtime(Path)},
    {ok, Ast} = beamai_jinja_parser:parse(Body, Opts),
    {ok, Forms, _} = beamai_jinja_compiler:forms(Ast, Opts),
...
t_source(Dir) ->
    Path = write(Dir, <<"hi {{ x }}">>),
    ok = build(Path, <<"hi {{ x }}">>),
    {ok, S} = beamai_jinja_dev:source(?MOD),
    ?assertMatch(#{path := _, stamp := _, mtime := _, vsn := _, opts := _}, S),
    ?assertEqual(list_to_binary(Path), maps:get(path, S)).

t_not_template() ->
    ?assertEqual({error, not_a_template_module}, beamai_jinja_dev:source(lists)).

t_fresh(Dir) ->
    Body = <<"hi {{ x }}">>,
    Path = write(Dir, Body),
    ok = build(Path, Body),
    ?assertEqual(false, beamai_jinja_dev:stale(?MOD)).
...
t_edited(Dir) ->
    ...
    ok = build(Path, <<"hi {{ x }}">>),
    _ = write(Dir, <<"bye {{ x }}">>),
    ?assertEqual(true, beamai_jinja_dev:stale(?MOD)).
...
t_reload(Dir) ->
    ...
    _ = write(Dir, <<"bye {{ x }}!">>),
    ok = beamai_jinja_dev:reload(?MOD),
...
t_reload_opts(Dir) ->
    Path = write(Dir, <<"{{ x }}">>),
    ok = build(Path, <<"{{ x }}">>),
    {ok, #{opts := Opts}} = beamai_jinja_dev:source(?MOD),
    ?assert(is_list(Opts)),
...
t_no_state(Dir) ->
    ...
    _ = write(Dir, <<"[{{ x }}]">>),
    ok = beamai_jinja_dev:reload(?MOD),
    ok = beamai_jinja_dev:reload(all),
...
t_listing(Dir) ->
    Path = write(Dir, <<"{{ x }}">>),
    ok = build(Path, <<"{{ x }}">>),
    ?assert(lists:member(?MOD, beamai_jinja_dev:template_modules())).
