...
%%% review.
%%%-------------------------------------------------------------------
-module(beamai_jinja_escape_tests).

-include_lib("eunit/include/eunit.hrl").

r(T)     -> beamai_jinja:render_string(T, #{}, #{}).
r(T, C)  -> beamai_jinja:render_string(T, C, #{}).
...
every_filter_declares_its_safe_class_test() ->
    Src = read_source("src/beamai_jinja_filters.erl"),
    Classes = ["transparent", "escaping", "generating", "none"],
    Missing = [N || N <- maps:keys(beamai_jinja_filters:filters()),
...
escape_can_be_turned_off_test() ->
    ?assertEqual(<<"<b>">>,
                 beamai_jinja:render_string(<<"{{ x }}">>, #{x => <<"<b>">>},
                                        #{escape => false})).
