...
builtin_registry_is_complete_test() ->
    {ok, #{filters := F, tests := T, modules := M}} = beamai_jinja_ext:registry([]),
    ?assertEqual(lists:sort(maps:keys(beamai_jinja_filters:filters())),
                 lists:sort(maps:keys(F))),
    ?assertEqual(lists:sort(maps:keys(beamai_jinja_tests:tests())),
...
extension_is_registered_test() ->
    {ok, #{filters := F, params := P, modules := M}} =
        beamai_jinja_ext:registry([beamai_jinja_ext_tests_ext]),
    ?assertMatch(#{money := {beamai_jinja_ext_tests_ext, money}}, F),
    ?assertEqual([currency], maps:get(money, P)),
    ?assert(lists:member(beamai_jinja_ext_tests_ext, M)).
...
shadowing_a_builtin_is_refused_test() ->
    ?assertMatch({error, {filter_name_conflict, upper, _}},
                 beamai_jinja_ext:registry([beamai_jinja_ext_tests_clash])).

two_extensions_claiming_one_name_test() ->
    ?assertMatch({error, {filter_name_conflict, money, _}},
                 beamai_jinja_ext:registry([beamai_jinja_ext_tests_ext,
                                        beamai_jinja_ext_tests_ext2])).

format_error_names_both_sides_test() ->
    Msg = beamai_jinja_ext:format_error({filter_name_conflict, upper, my_mod}),
    ?assert(is_list(Msg)),
...
custom_filter_is_usable_test() ->
    Opts = #{extensions => [beamai_jinja_ext_tests_ext]},
    ?assertEqual(<<"9 EUR">>,
                 beamai_jinja:render_string(<<"{{ 9|money('EUR') }}">>, #{}, Opts)),
    %% Positional and keyword forms have to reach the function identically.
    ?assertEqual(<<"9 EUR">>,
                 beamai_jinja:render_string(<<"{{ 9|money(currency='EUR') }}">>,
                                        #{}, Opts)).

unknown_filter_is_a_compile_error_test() ->
    ?assertError({beamai_jinja, {error, {_, _, {unknown_filter, nosuch}}}},
                 beamai_jinja:render_string(<<"{{ 1|nosuch }}">>, #{}, #{})),
    ?assertError({beamai_jinja, {error, {_, _, {unknown_test, nosuch}}}},
                 beamai_jinja:render_string(<<"{{ 1 is nosuch }}">>, #{}, #{})).
...
extension_module_is_allowed_in_generated_code_test() ->
    ...
    {ok, Nodes} = beamai_jinja_parser:parse(<<"{{ x|money('EUR') }}">>, Opts),
    ?assertMatch({ok, _, []}, beamai_jinja_compiler:forms(Nodes, Opts)).
...
spec_module_reads_a_loaded_module_test() ->
    {ok, #{filters := F, tests := T}} =
        beamai_jinja_ext:spec_module(beamai_jinja_ext_tests_ext),
    ?assertEqual([money], F),
    ?assertEqual([], T).
