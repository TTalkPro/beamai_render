...
%%%-------------------------------------------------------------------
-module(beamai_jinja_filters_tests).
...
filters_table_matches_the_exports_test() ->
    Declared = lists:sort(maps:keys(beamai_jinja_filters:filters())),
    Exported = lists:sort([N || {N, 2} <- beamai_jinja_filters:module_info(exports),
                                not lists:member(N, [filters, params, tests])]),
    ?assertEqual(Exported, Declared).

tests_table_matches_the_exports_test() ->
    Declared = lists:sort(maps:keys(beamai_jinja_tests:tests())),
    Exported = lists:sort([N || {N, 2} <- beamai_jinja_tests:module_info(exports),
                                not lists:member(N, [tests, params])]),
    ?assertEqual(Exported, Declared).

every_table_entry_points_at_a_real_function_test() ->
    [?assert(erlang:function_exported(M, F, 2))
     || {M, F} <- maps:values(beamai_jinja_filters:filters())
                      ++ maps:values(beamai_jinja_tests:tests())].
...
params_only_name_declared_filters_test() ->
    Known = maps:keys(beamai_jinja_filters:filters()),
    ?assertEqual([], [N || N <- maps:keys(beamai_jinja_filters:params()),
                           not lists:member(N, Known)]),
    KnownT = maps:keys(beamai_jinja_tests:tests()),
    ?assertEqual([], [N || N <- maps:keys(beamai_jinja_tests:params()),
...
no_filter_crashes_on_undefined_test() ->
    Bad = [N || N <- maps:keys(beamai_jinja_filters:filters()),
                not survives(fun() ->
                                     {M, F} = maps:get(N, beamai_jinja_filters:filters()),
                                     M:F(undefined, default_args(N))
                             end)],
    ?assertEqual([], Bad).

no_test_crashes_on_undefined_test() ->
    Bad = [N || N <- maps:keys(beamai_jinja_tests:tests()),
                not survives(fun() ->
                                     {M, F} = maps:get(N, beamai_jinja_tests:tests()),
...
survives(F) ->
    try _ = F(), true
    catch error:{beamai_jinja, _} -> true;       % a deliberate diagnosis is fine
          _:_ -> false
    end.
...
round_matches_python_test() ->
    R = fun(V, P) -> beamai_jinja_filters:round(V, #{precision => P}) end,
    ?assertEqual(0.0, R(0.5, 0)),
    ?assertEqual(2.0, R(1.5, 0)),
    ?assertEqual(2.0, R(2.5, 0)),
    ?assertEqual(2.35, R(2.345, 2)),
    ?assertEqual(-0.0, R(-0.5, 0)),
    ?assertEqual(2.0, beamai_jinja_filters:round(1.4, #{method => <<"ceil">>})),
    ?assertEqual(1.0, beamai_jinja_filters:round(1.6, #{method => <<"floor">>})).
...
tojson_is_html_safe_test() ->
    {safe, Out} = beamai_jinja_filters:tojson(#{a => <<"</script>">>}, #{}),
...
filters_source() ->
    ...
    {ok, Bin} = file:read_file(up(Dir, "src/beamai_jinja_filters.erl", 6)),
...
defined_is_false_for_none_test() ->
    ?assertNot(beamai_jinja_tests:defined(undefined, #{})),
    ?assert(beamai_jinja_tests:defined(false, #{})),
    ?assert(beamai_jinja_tests:none(undefined, #{})).
