...
%%%-------------------------------------------------------------------
-module(beamai_jinja_inline_tests).
...
expanded(T, C) ->
    ...
    case beamai_jinja_parser:parse(T, Opts) of
        {error, {_, _, R}} -> {error, tag(R)};
        {ok, Nodes} ->
            case beamai_jinja_compiler:compile_inline(Nodes, erl_parse:abstract(C),
                                                  Opts) of
                {error, {_, _, R}} -> {error, tag(R)};
                {ok, Expr} ->
...
interpreted(T, C) ->
    try {ok, beamai_jinja:inline(T, C)}
    catch error:{beamai_jinja, {error, {_, _, R}}} -> {error, tag(R)};
          error:{beamai_jinja, R}                  -> {error, tag(R)}
    end.
...
forbidden_statements_test() ->
    [?assertEqual(true, beamai_jinja_compiler:inline_forbidden(K))
     || K <- [include, extends, import, from, block]],
    [?assertEqual(false, beamai_jinja_compiler:inline_forbidden(K))
...
generated_variables_do_not_collide_test() ->
    Opts = #{module => j2_inline_probe, source => <<"inline">>},
    {ok, Nodes} = beamai_jinja_parser:parse(
                    <<"{% for x in xs %}{{ loop.index }}{% endfor %}">>, Opts),
    {ok, Expr} = beamai_jinja_compiler:compile_inline(
