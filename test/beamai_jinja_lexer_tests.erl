...
%%% end.
%%%-------------------------------------------------------------------
-module(beamai_jinja_lexer_tests).

-include_lib("eunit/include/eunit.hrl").

lex(Bin) -> beamai_jinja_lexer:tokens(Bin, {1, 1}, expr_end).
lex(Bin, Stop) -> beamai_jinja_lexer:tokens(Bin, {1, 1}, Stop).
...
columns_are_tracked_test() ->
    {ok, T, _, _, _} = beamai_jinja_lexer:tokens(<<"a + bb }}">>, {1, 3}, expr_end),
    ?assertMatch([{name, {1, 3}, a}, {op, {1, 5}, '+'}, {name, {1, 7}, bb}], T).

newlines_inside_a_tag_advance_the_line_test() ->
    {ok, T, _, _, _} = beamai_jinja_lexer:tokens(<<"a\n  b }}">>, {1, 3}, expr_end),
