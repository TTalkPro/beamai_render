...
%%%-------------------------------------------------------------------
-module(beamai_jinja_parser_tests).

-include_lib("eunit/include/eunit.hrl").

p(Bin) -> beamai_jinja_parser:parse(Bin, #{source => <<"t">>}).
...
filter_block_is_a_chain_test() ->
    Body = beamai_jinja_parser:body_placeholder(),
...
extends_warnings_test() ->
    ...
    W = beamai_jinja_parser:warnings(Nodes),
...
blank_text_after_extends_is_fine_test() ->
    ...
    ?assertEqual([], beamai_jinja_parser:warnings(Nodes)).

shadowing_a_reserved_name_warns_test() ->
    Nodes = ok_(<<"{% for loop in xs %}x{% endfor %}">>),
    ?assertMatch([{_, {shadows_reserved, loop}}], beamai_jinja_parser:warnings(Nodes)),
    ?assertMatch([{_, {shadows_reserved, caller}}],
                 beamai_jinja_parser:warnings(ok_(<<"{% set caller = 1 %}">>))).
