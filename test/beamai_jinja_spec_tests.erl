...
%%% @end
%%%-------------------------------------------------------------------
-module(beamai_jinja_spec_tests).
...
inventory_test_() ->
    [{"group " ++ binary_to_list(G) ++ " has at least " ++ integer_to_list(Min)
      ++ " cases",
      fun() ->
              N = length(beamai_jinja_test_lib:cases(G)),
              ?assert(N >= Min)
      end}
     || {G, Min} <- beamai_jinja_test_lib:inventory()].

total_inventory_test() ->
    ?assert(length(beamai_jinja_test_lib:all_cases()) >= 500).

case_names_are_unique_test() ->
    Names = [{maps:get(group, C), maps:get(name, C)}
             || C <- beamai_jinja_test_lib:all_cases()],
    ?assertEqual(lists:usort(Names), lists:sort(Names)).

every_case_has_an_expectation_test() ->
    Bad = [beamai_jinja_test_lib:title(C)
           || C <- beamai_jinja_test_lib:all_cases(),
...
every_deviation_has_a_fixture_test() ->
    Seen = lists:usort([D || C <- beamai_jinja_test_lib:all_cases(),
...
deviation_numbers_are_declared_test() ->
    Seen = [D || C <- beamai_jinja_test_lib:all_cases(),
...
every_reason_has_a_fixture_test() ->
    Seen = lists:usort([R || C <- beamai_jinja_test_lib:all_cases(),
...
conformance_test_() ->
    [{beamai_jinja_test_lib:title(C), fun() -> assert_case(C) end}
     || C <- beamai_jinja_test_lib:all_cases()].
...
assert_case(C) ->
    case beamai_jinja_test_lib:due(C) of
        false -> ok;
        true  -> assert_due(C)
    end.

pending_count_test() ->
    All = beamai_jinja_test_lib:all_cases(),
    Pending = [C || C <- All, not beamai_jinja_test_lib:due(C)],
    case Pending of
        [] -> ok;
        _  -> ?debugFmt("jinja spec: ~p of ~p cases pending at stage ~p",
                        [length(Pending), length(All),
                         beamai_jinja_test_lib:current_stage()])
    end.

assert_due(#{error := Reason} = C) ->
    ?assertEqual({error, Reason}, beamai_jinja_test_lib:run(C));
assert_due(#{expected := Expected} = C) ->
    ?assertEqual({ok, Expected}, beamai_jinja_test_lib:run(C)).
