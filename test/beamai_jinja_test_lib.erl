...
run(#{template := T, templates := Ts, context := Ctx}) ->
    try beamai_jinja:render_string(T, Ctx, #{templates => Ts}) of
        Bin when is_binary(Bin) -> {ok, Bin}
    catch
        error:{beamai_jinja, {error, {_File, _Line, Reason}}} -> {error, reason(Reason)};
        error:{beamai_jinja, Reason}                          -> {error, reason(Reason)};
        error:{beamai_html, Reason}                           -> {error, reason(Reason)}
    end.
