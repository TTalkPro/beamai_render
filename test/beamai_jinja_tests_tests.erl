%%%-------------------------------------------------------------------
%%% Tests for the facade, beamai_jinja.
%%%-------------------------------------------------------------------
-module(beamai_jinja_tests_tests).
...
render_is_a_direct_call_test() ->
    Mod = beamai_jinja_facade_probe,
    Opts = #{module => Mod, source => <<"t.j2">>},
    {ok, Nodes} = beamai_jinja_parser:parse(<<"hi {{ n }}">>, Opts),
    {ok, Forms, _} = beamai_jinja_compiler:forms(Nodes, Opts),
    {ok, Mod, Bin} = compile:forms(Forms, [return_errors, binary]),
    {module, Mod} = code:load_binary(Mod, "t.j2", Bin),
    ?assertEqual(<<"hi a">>, beamai_jinja:render(Mod, #{n => <<"a">>})),
    ?assertEqual(<<"hi a">>,
                 iolist_to_binary(beamai_jinja:render_iolist(Mod, #{n => <<"a">>}))),
...
compiled_sets_are_reused_test() ->
    ...
    {T1, _} = timer:tc(fun() -> beamai_jinja:render_string(T, #{n => 1}, #{}) end),
    {T2, _} = timer:tc(fun() -> beamai_jinja:render_string(T, #{n => 1}, #{}) end),
    ?assert(T2 * 4 < T1 orelse T2 < 200).

%% Two sets that both define `base.j2' must not collide.
template_sets_are_independent_test() ->
    A = beamai_jinja:render_string(<<"{% include \"b.j2\" %}">>, #{},
                               #{templates => #{<<"b.j2">> => <<"A">>}}),
    B = beamai_jinja:render_string(<<"{% include \"b.j2\" %}">>, #{},
                               #{templates => #{<<"b.j2">> => <<"B">>}}),
    ?assertEqual(<<"A">>, A),
    ?assertEqual(<<"B">>, B).
...
options_are_part_of_the_set_identity_test() ->
    T = <<"{{ n }}">>,
    ?assertEqual(<<"&lt;b&gt;">>, beamai_jinja:render_string(T, #{n => <<"<b>">>}, #{})),
    ?assertEqual(<<"<b>">>,
                 beamai_jinja:render_string(T, #{n => <<"<b>">>}, #{escape => false})).

errors_carry_the_engine_tag_test() ->
    ?assertError({beamai_jinja, {error, {_, _, {unclosed_block, 'if', _}}}},
                 beamai_jinja:render_string(<<"{% if x %}">>, #{}, #{})),
    ?assertError({beamai_jinja, {not_renderable, _}},
                 beamai_jinja:render_string(<<"{{ n }}">>, #{n => self()}, #{})).

%% A module compiled from a string has no file to go stale against, so it is
%% marked and beamai_jinja_dev skips it.
string_templates_are_marked_test() ->
    _ = beamai_jinja:render_string(<<"marked {{ n }}">>, #{n => 1}, #{}),
    Marked = [M || M <- beamai_jinja_dev:template_modules(),
                   {ok, S} <- [beamai_jinja_dev:source(M)],
                   maps:get(origin, S, file) =:= string],
    ?assert(length(Marked) > 0),
    ?assertEqual(ok, beamai_jinja_dev:check()).
