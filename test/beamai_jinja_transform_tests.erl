%%%-------------------------------------------------------------------
%%% @doc Tests for beamai_jinja_transform: the three source forms.
%%%
...
%%% @end
%%%-------------------------------------------------------------------
-module(beamai_jinja_transform_tests).

-include_lib("eunit/include/eunit.hrl").

%% A second parse_transform, used to prove beamai_jinja_transform does not assume
...
%% module below, once at run time by beamai_jinja:inline/2. The outputs have to
%% match byte for byte.
diff_cases() ->
    [{<<"plain">>, #{}},
     {<<"{{ n }}">>, #{n => <<"<b>">>}},
...
expansion_matches_the_fallback_test_() ->
    ...
    [{binary_to_list(T),
      ?_assertEqual(beamai_jinja:inline(T, C), Mod:F(C))}
...
inline_include_is_an_error_test() ->
    ...
    ?assertError({beamai_jinja, {error, {_, _, {target_in_inline_template, _}}}},
                 beamai_jinja:inline(<<"{% include \"x.j2\" %}">>, #{})).
...
missing_template_lists_the_directories_test() ->
    ...
    Msg = beamai_jinja_transform:format_error(
...
no_jinja_forms_is_a_no_op_test() ->
    ...
    ?assertEqual(Forms, beamai_jinja_transform:parse_transform(Forms, [])).
...
idempotent_test() ->
    ...
    Once = strip(beamai_jinja_transform:parse_transform(Forms, [nowarn_jinja_inline])),
    Twice = strip(beamai_jinja_transform:parse_transform(Once, [nowarn_jinja_inline])),
