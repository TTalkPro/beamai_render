...
%%% @end
%%%-------------------------------------------------------------------
-module(beamai_mustache_ast_tests).
...
ast(Bin) ->
    case beamai_mustache_parser:parse(Bin) of
        {ok, Ns}       -> Ns;
...
merge_text_joins_adjacent_text_test() ->
    ?assertEqual([{text, {1, 1}, <<"ab">>}],
                 beamai_mustache_ast:merge_text(ast(<<"a{{!c}}b">>))).

merge_text_joins_a_long_run_test() ->
    ?assertEqual([{text, {1, 1}, <<"abcd">>}],
                 beamai_mustache_ast:merge_text(ast(<<"a{{!1}}b{{!2}}c{{!3}}d">>))).
...
merge_text_keeps_the_first_fragments_loc_test() ->
    ...
    ?assertEqual([{text, {7, 3}, <<"abc">>}], beamai_mustache_ast:merge_text(Nodes)).

merge_text_stops_at_a_non_text_node_test() ->
    ?assertEqual([{text, {1, 1}, <<"ab">>},
                  {var, {1, 3}, [x], escape},
                  {text, {1, 8}, <<"cd">>}],
                 beamai_mustache_ast:merge_text(
                   [txt({1, 1}, <<"a">>), txt({1, 2}, <<"b">>),
                    {var, {1, 3}, [x], escape},
                    txt({1, 8}, <<"c">>), txt({1, 9}, <<"d">>)])).
...
merge_text_leaves_a_single_text_alone_test() ->
    ?assertEqual([{text, {1, 1}, <<"abc">>}],
                 beamai_mustache_ast:merge_text([txt({1, 1}, <<"abc">>)])).

merge_text_on_empty_input_test() ->
    ?assertEqual([], beamai_mustache_ast:merge_text([])).

merge_text_is_idempotent_test() ->
    Nodes = ast(<<"a{{!c}}b{{#s}}x{{!c}}y{{/s}}z{{!c}}w">>),
    Once  = beamai_mustache_ast:merge_text(Nodes),
    ?assertEqual(Once, beamai_mustache_ast:merge_text(Once)).
...
p2_regression_merges_inside_a_has_body_test() ->
    ?assertEqual([{has, {1, 1}, [x], [{text, {1, 7}, <<"ab">>}], true}],
                 beamai_mustache_ast:merge_text(ast(<<"{{+x}}a{{! c }}b{{/x}}">>))).
...
p2_regression_merges_inside_an_ext_body_test() ->
    ...
    ?assertEqual([{ext, ?L, $@, [k], [{text, {2, 1}, <<"ab">>}]}],
                 beamai_mustache_ast:merge_text([Node])).
...
p1_regression_merge_text_is_linear_test_() ->
    {timeout, 30,
     fun() ->
             ...
             {Micros, Merged} = timer:tc(fun() -> beamai_mustache_ast:merge_text(Nodes) end),
...
drop_empty_removes_an_empty_has_test() ->
    ?assertEqual([], beamai_mustache_ast:drop_empty(ast(<<"{{+x}}{{/x}}">>))),
    ?assertEqual([], beamai_mustache_ast:drop_empty(ast(<<"{{-x}}{{/x}}">>))).

drop_empty_keeps_a_non_empty_has_test() ->
    ?assertEqual([{has, {1, 1}, [x], [{text, {1, 7}, <<"y">>}], true}],
                 beamai_mustache_ast:drop_empty(ast(<<"{{+x}}y{{/x}}">>))).
...
drop_empty_keeps_an_empty_section_because_of_fun2_lambdas_test() ->
    ?assertEqual([{section, {1, 1}, [w], []}],
                 beamai_mustache_ast:drop_empty(ast(<<"{{#w}}{{/w}}">>))).
...
drop_empty_keeps_an_empty_ext_test() ->
    ?assertEqual([ext([])], beamai_mustache_ast:drop_empty([ext([])])).
...
drop_empty_leaves_leaf_nodes_alone_test() ->
    Nodes = [txt(?L, <<"a">>),
             ...
             {lambda, ?L, [f]},
             {partial, ?L, <<"p">>, <<>>}],
    ?assertEqual(Nodes, beamai_mustache_ast:drop_empty(Nodes)).
...
drop_empty_cascades_upwards_test() ->
    ?assertEqual([], beamai_mustache_ast:drop_empty(
                       ast(<<"{{+a}}{{+b}}{{/b}}{{/a}}">>))).

drop_empty_is_idempotent_test() ->
    Nodes = ast(<<"{{#a}}{{+b}}{{/b}}x{{/a}}{{+c}}{{/c}}">>),
    Once  = beamai_mustache_ast:drop_empty(Nodes),
    ?assertEqual(Once, beamai_mustache_ast:drop_empty(Once)).
...
module_name_test_() ->
    [{Title, ?_assertEqual(Expected, beamai_mustache_ast:module_name(Input, #{}))}
     || {Title, Input, Expected} <- module_name_cases()].

module_name_honours_the_prefix_option_test() ->
    ?assertEqual(tpl_shared_item,
                 beamai_mustache_ast:module_name(<<"shared/item">>,
                                             #{prefix => <<"tpl_">>})),
...
default_prefix_test() ->
    ?assertEqual(<<"view_">>, beamai_mustache_ast:default_prefix()).
...
resolve_partials_rewrites_the_target_test() ->
    ?assertEqual({ok, [{partial, {1, 1}, view_shared_item, <<>>},
                 ...
                 beamai_mustache_ast:resolve_partials(
...
resolve_partials_honours_the_prefix_option_test() ->
    ?assertEqual({ok, [{partial, {1, 1}, tpl_index, <<>>}], [tpl_index]},
                 beamai_mustache_ast:resolve_partials(ast(<<"{{>index}}">>),
                                                  #{prefix => <<"tpl_">>})).

resolve_partials_keeps_the_indent_test() ->
    ?assertEqual({ok, [{partial, {1, 3}, view_p, <<"  ">>}], [view_p]},
                 beamai_mustache_ast:resolve_partials(ast(<<"  {{> p}}\n">>), #{})).
...
resolve_partials_descends_into_every_body_type_test() ->
    Check = fun(Bin) ->
                    {ok, _, Deps} =
                        beamai_mustache_ast:resolve_partials(ast(Bin), #{}),
                    Deps
            end,
...
resolve_partials_without_partials_test() ->
    Nodes = beamai_mustache_ast:merge_text(ast(<<"a{{x}}b">>)),
    ?assertEqual({ok, Nodes, []}, beamai_mustache_ast:resolve_partials(Nodes, #{})).
...
postprocess_is_the_three_passes_in_order_test() ->
    ...
    Expected = beamai_mustache_ast:resolve_partials(
                 beamai_mustache_ast:drop_empty(
                   beamai_mustache_ast:merge_text(Nodes)), #{}),
    ?assertEqual(Expected, beamai_mustache_ast:postprocess(Nodes, #{})).

postprocess_result_shape_test() ->
    ?assertEqual({ok, [{text, {1, 1}, <<"ab">>},
                       {partial, {1, 9}, view_p, <<>>}],
                  [view_p]},
                 beamai_mustache_ast:postprocess(ast(<<"a{{!c}}b{{>p}}">>), #{})).

postprocess_propagates_a_partial_error_test() ->
    Dir = filename:join(test_dir(), "no_such_views_dir"),
    ?assertMatch({error, {_, 1, {partial_not_found, <<"gone">>}}},
                 beamai_mustache_ast:postprocess(ast(<<"a{{>gone}}">>),
                                             #{views => Dir})).

postprocess_is_idempotent_on_the_node_list_test() ->
    Nodes = ast(<<"a{{!c}}b{{#s}}x{{!c}}y{{+e}}{{/e}}{{/s}}">>),
    {ok, Once, Deps1} = beamai_mustache_ast:postprocess(Nodes, #{}),
    {ok, Twice, Deps2} = beamai_mustache_ast:postprocess(Once, #{}),
...
postprocess_is_byte_stable_test() ->
    ...
    {ok, N1, D1} = beamai_mustache_ast:postprocess(Nodes, #{}),
    {ok, N2, D2} = beamai_mustache_ast:postprocess(Nodes, #{}),
    ?assertEqual(term_to_binary({N1, D1}), term_to_binary({N2, D2})).

postprocess_is_byte_stable_across_reparses_test() ->
    Bin = <<"x\n{{#s}}\n  {{>shared/item}}\n{{/s}}\n{{! done }}\n">>,
    Run = fun() ->
                  {ok, N, D} = beamai_mustache_ast:postprocess(ast(Bin), #{}),
                  term_to_binary({N, D})
          end,
