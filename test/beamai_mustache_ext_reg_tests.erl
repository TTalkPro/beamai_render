%%%-------------------------------------------------------------------
%%% @doc Tests for beamai_mustache_ext: the extension behaviour, the validation
...
%%% @end
%%%-------------------------------------------------------------------
-module(beamai_mustache_ext_reg_tests).
...
builtin_markers_cases() ->
    [{"builtin markers come from the header, not a second copy",
      fun() ->
              ?assertEqual(?BEAMAI_MUSTACHE_BUILTIN_MARKERS,
                           beamai_mustache_ext:builtin_markers())
      end},
     {"every builtin marker is reserved",
      fun() ->
              [?assert(beamai_mustache_ext:is_reserved(C))
               || C <- ?BEAMAI_MUSTACHE_BUILTIN_MARKERS]
      end},
     {"the reserved set is exactly the documented characters",
      fun() ->
              ?assertEqual(lists:sort("#^/>!=&{}+-*"),
                           lists:sort(beamai_mustache_ext:builtin_markers()))
      end},
     {"a character outside the set is not reserved",
      fun() -> ?assertNot(beamai_mustache_ext:is_reserved($@)) end}].
...
valid_marker_cases() ->
    ...
    [{"punctuation is usable as a marker",
      fun() ->
              [?assert(beamai_mustache_ext:is_valid_marker(C))
               || C <- Good, not beamai_mustache_ext:is_reserved(C)]
      end},
     {"braces, whitespace, word characters and non-ASCII are not",
      fun() ->
              [?assertNot(beamai_mustache_ext:is_valid_marker(C)) || C <- Bad]
      end}].
    ...
    [{"a bare module claims every marker it declares",
      fun() ->
              ?assertEqual({ok, #{$@ => my_i18n, $% => my_wrap}},
                           beamai_mustache_ext:registry([my_i18n, my_wrap]))
      end},
     {"a {Marker, Module} spec claims exactly one character",
      fun() ->
              ?assertEqual({ok, #{$@ => my_i18n}},
                           beamai_mustache_ext:registry([{$@, my_i18n}]))
      end},
     {"a {Marker, {Module, Fun}} spec is accepted too",
      fun() ->
              ?assertEqual({ok, #{$@ => my_i18n}},
                           beamai_mustache_ext:registry([{$@, {my_i18n, compile_tag}}]))
      end},
     ...
     {"two modules claiming the same marker lists both",
      fun() ->
              ?assertEqual({error, {marker_conflict, $~,
                                    [bad_ext_conflict_a, bad_ext_conflict_b]}},
                           beamai_mustache_ext:registry([bad_ext_conflict_a,
                                                     bad_ext_conflict_b]))
      end},
     {"the conflict report does not depend on the input order",
      fun() ->
              A = beamai_mustache_ext:registry([bad_ext_conflict_a,
                                            bad_ext_conflict_b]),
              B = beamai_mustache_ext:registry([bad_ext_conflict_b,
                                            bad_ext_conflict_a]),
              ?assertEqual(A, B)
      end},
     {"the same module listed twice is not a conflict with itself",
      fun() ->
              ?assertEqual({ok, #{$@ => my_i18n}},
                           beamai_mustache_ext:registry([my_i18n, my_i18n]))
      end},
     ...
     {"an empty list yields an empty table",
      fun() -> ?assertEqual({ok, #{}}, beamai_mustache_ext:registry([])) end},
     {"spec_module/1 and spec_markers/1 read the three shapes",
      fun() ->
              ?assertEqual(my_i18n, beamai_mustache_ext:spec_module(my_i18n)),
              ?assertEqual(my_i18n, beamai_mustache_ext:spec_module({$@, my_i18n})),
              ?assertEqual(my_i18n,
                           beamai_mustache_ext:spec_module({$@, {my_i18n, f}})),
              ?assertEqual(all, beamai_mustache_ext:spec_markers(my_i18n)),
              ?assertEqual([$@], beamai_mustache_ext:spec_markers({$@, my_i18n}))
      end}].
...
src_only_dir() ->
    Dir = filename:join(code:lib_dir(beamai_render), "tmp_transform_tests"),
...
helper_cases() ->
    [{"the stack variable defaults to S and is overridable",
      fun() ->
              ?assertEqual('S', beamai_mustache_ext:stack_var(#{})),
              ?assertEqual('S__7', beamai_mustache_ext:stack_var(#{stack_var => 'S__7'})),
              ?assertEqual('I', beamai_mustache_ext:indent_var(#{}))
      end},
     {"stack_expr/1 tracks the configured name instead of hard-coding S",
      fun() ->
              {var, _, V} = beamai_mustache_ext:stack_expr(#{stack_var => 'Z'}),
              ?assertEqual('Z', V),
              {var, _, D} = beamai_mustache_ext:stack_expr(#{}),
              ?assertEqual('S', D)
      end},
     {"anno/1 uses loc when it is there and 0 when it is not",
      fun() ->
              ?assertEqual(3, erl_anno:line(beamai_mustache_ext:anno(#{loc => {3, 5}}))),
              ?assertEqual(0, erl_anno:line(beamai_mustache_ext:anno(#{})))
      end},
     {"lookup_expr/2 builds the runtime call and evaluates correctly",
      fun() ->
              E = beamai_mustache_ext:lookup_expr([a, b], #{}),
              ?assertMatch({call, _, {remote, _, {atom, _, beamai_mustache_rt},
     ...
     {"escape_expr/2 and to_binary_expr/2 build the runtime calls",
      fun() ->
              ...
              ?assertEqual(<<"&lt;a&gt;">>,
                           iolist_to_binary(
                             eval_with_stack(
                               beamai_mustache_ext:escape_expr(Lit, #{}), []))),
              ?assertEqual(<<"<a>">>,
                           eval_with_stack(beamai_mustache_ext:to_binary_expr(Lit, #{}), []))
      end},
     {"ext_opt/3 reads the module's own slice of ext_opts",
      fun() ->
              ?assertEqual(dflt, beamai_mustache_ext:ext_opt(my_i18n, #{}, dflt)),
              ?assertEqual(cfg,
                           beamai_mustache_ext:ext_opt(
...
format_error_cases() ->
    Reasons =
        [{{ext_module_not_found, my_ext}, ["my_ext", "erl_first_files"]},
         {{not_an_ext_module, my_ext},    ["my_ext", "beamai_mustache_ext"]},
    ...
    [{lists:flatten(io_lib:format("format_error ~p mentions ~p", [R, Words])),
      fun() ->
              Text = lists:flatten(beamai_mustache_ext:format_error(R)),
