%%%-------------------------------------------------------------------
%%% @doc Unit tests for beamai_mustache_parser.
%%%
%%% The parser turns a token stream into the raw AST: no merging, no pruning,
%%% no partial resolution (those are beamai_mustache_ast's three passes, tested in
%%% beamai_mustache_ast_tests). So the assertions here are about node shape and
...
%%% @end
%%%-------------------------------------------------------------------
-module(beamai_mustache_parser_tests).
...
ast(Input) ->
    case beamai_mustache_parser:parse(Input) of
        {ok, Ns}       -> Ns;
...
parse_error(Input) ->
    Got = try
              beamai_mustache_parser:parse(Input)
          catch
              Cl:Rs:Stack ->
...
parse_accepts_a_token_list_test() ->
    ...
    ?assertEqual({ok, [{text, {1, 1}, <<"a">>},
                 ...
                 beamai_mustache_parser:parse(Tokens, #{})).

parse_1_accepts_a_token_list_test() ->
    ?assertEqual({ok, [{var, {3, 7}, [x], escape}]},
                 beamai_mustache_parser:parse([{tag, {3, 7}, none, <<"x">>, <<>>}])).

parse_empty_token_list_test() ->
    ?assertEqual({ok, []}, beamai_mustache_parser:parse([])),
    ?assertEqual({ok, []}, beamai_mustache_parser:parse(<<>>)).
...
parse_preserves_token_locations_test() ->
    ?assertEqual({ok, [{section, {2, 5}, [a], [{text, {3, 1}, <<"b">>}]}]},
                 beamai_mustache_parser:parse([{tag,  {2, 5}, $#, <<"a">>, <<>>},
                                           {text, {3, 1}, <<"b">>},
                                           {tag,  {4, 1}, $/, <<"a">>, <<>>}])).
...
b4_regression_partial_name_is_not_a_key_path_test() ->
    ?assertMatch({ok, [{partial, _, <<"layout.default">>, _}]},
                 beamai_mustache_parser:parse(<<"{{> layout.default}}">>)),
    ?assertMatch({ok, [{partial, _, <<"shared/item">>, _}]},
                 beamai_mustache_parser:parse(<<"{{> shared/item}}">>)),
    ?assertMatch({ok, [{partial, _, <<"a/b/c.d-e">>, _}]},
                 beamai_mustache_parser:parse(<<"{{> a/b/c.d-e}}">>)).
...
b4_regression_partial_inside_a_section_test() ->
    ?assertMatch({ok, [{section, _, [s], [{partial, _, <<"layout.default">>, _}]}]},
                 beamai_mustache_parser:parse(<<"{{#s}}{{> layout.default}}{{/s}}">>)).

%% B5 regression: {{.}} is the implicit iterator and must produce the ['.']
%% sentinel that beamai_mustache_rt:lookup/2 recognises, not an empty key list.
b5_regression_implicit_iterator_test() ->
    ?assertEqual({ok, [{var, {1, 1}, [?BEAMAI_MUSTACHE_DOT], escape}]},
                 beamai_mustache_parser:parse(<<"{{.}}">>)),
    ?assertEqual({ok, [{var, {1, 1}, [?BEAMAI_MUSTACHE_DOT], raw}]},
                 beamai_mustache_parser:parse(<<"{{{.}}}">>)),
    ?assertEqual({ok, [{var, {1, 1}, [?BEAMAI_MUSTACHE_DOT], raw}]},
                 beamai_mustache_parser:parse(<<"{{&.}}">>)).
...
b5_regression_implicit_iterator_inside_a_section_test() ->
    ?assertEqual([{section, ?L, [a], [{var, ?L, [?BEAMAI_MUSTACHE_DOT], escape}]}],
                 strip_loc(ast(<<"{{#a}}{{.}}{{/a}}">>))).
...
keys_cases() ->
    [{"a dotted name splits into segments", <<"a.b">>,     [a, b]},
     ...
     {"a single name",                      <<"a">>,       [a]},
     {"a lone dot is the implicit iterator", <<".">>,      [?BEAMAI_MUSTACHE_DOT]},
     {"empty content is the implicit iterator", <<>>,      [?BEAMAI_MUSTACHE_DOT]},
...
keys_test_() ->
    [{Title, ?_assertEqual(Expected, beamai_mustache_parser:keys(Input))}
...
partial_name_test_() ->
    [{Title, ?_assertEqual(Expected, beamai_mustache_parser:partial_name(Input))}
