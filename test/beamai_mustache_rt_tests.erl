%%%-------------------------------------------------------------------
%%% @doc Unit tests for beamai_mustache_rt.
%%%
%%% These exercise the runtime functions directly rather than through
%%% beamai_mustache: the facade and the compiler do not exist yet, and the
...
%%% @end
%%%-------------------------------------------------------------------
-module(beamai_mustache_rt_tests).

-include_lib("eunit/include/eunit.hrl").
-include("beamai_mustache.hrl").
...
lookup_single_segment_test() ->
    ?assertEqual(1, beamai_mustache_rt:lookup([a], [#{a => 1}])).

lookup_multi_segment_test() ->
    ?assertEqual(2, beamai_mustache_rt:lookup([a, b], [#{a => #{b => 2}}])).

lookup_deep_segment_test() ->
    Ctx = #{a => #{b => #{c => 3}}},
    ?assertEqual(3, beamai_mustache_rt:lookup([a, b, c], [Ctx])).

%% The first segment walks outwards through the stack.
lookup_first_segment_backtracks_test() ->
    ?assertEqual(2, beamai_mustache_rt:lookup([a], [#{b => 1}, #{a => 2}])).

%% ... and stops at the first frame that has the key, innermost wins.
lookup_innermost_frame_wins_test() ->
    ?assertEqual(1, beamai_mustache_rt:lookup([a], [#{a => 1}, #{a => 2}])).
...
lookup_rest_does_not_backtrack_test() ->
    ?assertEqual(undefined,
                 beamai_mustache_rt:lookup([a, b], [#{a => #{c => 1}}, #{b => 2}])).

lookup_rest_through_non_map_test() ->
    ?assertEqual(undefined, beamai_mustache_rt:lookup([a, b], [#{a => 7}])).

%% {{.}} is the top frame itself.
lookup_dot_test() ->
    ?assertEqual(<<"v">>,
                 beamai_mustache_rt:lookup([?BEAMAI_MUSTACHE_DOT], [<<"v">>, #{a => 1}])).

lookup_dot_map_frame_test() ->
    M = #{a => 1},
    ?assertEqual(M, beamai_mustache_rt:lookup([?BEAMAI_MUSTACHE_DOT], [M])).

lookup_dot_empty_stack_test() ->
    ?assertEqual(undefined, beamai_mustache_rt:lookup([?BEAMAI_MUSTACHE_DOT], [])).
...
lookup_skips_non_map_frames_test() ->
    ?assertEqual(1, beamai_mustache_rt:lookup([a], [<<"x">>, #{a => 1}])),
    ?assertEqual(1, beamai_mustache_rt:lookup([a], [42, [1, 2], #{a => 1}])).

%% The root context is allowed to be a bare scalar (spec: implicit iterator
%% over the integer 85).
lookup_bare_scalar_root_test() ->
    ?assertEqual(85, beamai_mustache_rt:lookup([?BEAMAI_MUSTACHE_DOT], [85])),
    ?assertEqual(undefined, beamai_mustache_rt:lookup([a], [85])).

lookup_missing_test() ->
    ?assertEqual(undefined, beamai_mustache_rt:lookup([a], [])),
    ?assertEqual(undefined, beamai_mustache_rt:lookup([a], [#{}])),
    ?assertEqual(undefined, beamai_mustache_rt:lookup([a], [#{b => 1}, #{c => 2}])).
...
lookup_stored_undefined_test() ->
    ?assertEqual(undefined, beamai_mustache_rt:lookup([a], [#{a => undefined}])).

lookup_empty_keys_test() ->
    ?assertEqual(#{a => 1}, beamai_mustache_rt:lookup([], [#{a => 1}])),
    ?assertEqual(undefined, beamai_mustache_rt:lookup([], [])).
...
truthy_falsy_set_test() ->
    ?assertEqual(false, beamai_mustache_rt:truthy(undefined)),
    ?assertEqual(false, beamai_mustache_rt:truthy(false)),
    ?assertEqual(false, beamai_mustache_rt:truthy([])),
    ?assertEqual(false, beamai_mustache_rt:truthy(<<>>)),
    ?assertEqual(false, beamai_mustache_rt:truthy(null)).

truthy_zero_is_true_test() ->
    ?assertEqual(true, beamai_mustache_rt:truthy(0)),
    ?assertEqual(true, beamai_mustache_rt:truthy(0.0)).

truthy_empty_map_is_true_test() ->
    ?assertEqual(true, beamai_mustache_rt:truthy(#{})).

truthy_other_values_test() ->
    ?assertEqual(true, beamai_mustache_rt:truthy(true)),
    ?assertEqual(true, beamai_mustache_rt:truthy(<<"a">>)),
    ?assertEqual(true, beamai_mustache_rt:truthy([1])),
    ?assertEqual(true, beamai_mustache_rt:truthy(atom)).
...
to_binary_integer_test() ->
    ?assertEqual(<<"85">>, beamai_mustache_rt:to_binary(85)),
    ?assertEqual(<<"0">>, beamai_mustache_rt:to_binary(0)).

to_binary_negative_test() ->
    ?assertEqual(<<"-7">>, beamai_mustache_rt:to_binary(-7)).
...
to_binary_float_short_test() ->
    ...
    ?assertEqual(<<"1.1">>, beamai_mustache_rt:to_binary(1.1)),
    ?assertEqual(<<"0.0">>, beamai_mustache_rt:to_binary(0.0)),
    ?assertEqual(<<"-1.5">>, beamai_mustache_rt:to_binary(-1.5)).

to_binary_binary_is_identity_test() ->
    B = <<"hello">>,
    ?assert(beamai_mustache_rt:to_binary(B) =:= B),
    ?assert(erts_debug:same(beamai_mustache_rt:to_binary(B), B)).

to_binary_atom_test() ->
    ?assertEqual(<<"abc">>, beamai_mustache_rt:to_binary(abc)),
    ?assertEqual(<<"true">>, beamai_mustache_rt:to_binary(true)),
    ?assertEqual(<<"false">>, beamai_mustache_rt:to_binary(false)).

to_binary_undefined_and_null_test() ->
    ?assertEqual(<<>>, beamai_mustache_rt:to_binary(undefined)),
    ?assertEqual(<<>>, beamai_mustache_rt:to_binary(null)).

to_binary_list_test() ->
    ?assertEqual(<<"hi">>, beamai_mustache_rt:to_binary("hi")),
    ?assertEqual(<<"ab">>, beamai_mustache_rt:to_binary([<<"a">>, [<<"b">>]])),
    ?assertEqual(<<>>, beamai_mustache_rt:to_binary([])).

to_binary_utf8_test() ->
    ?assertEqual(<<"中文"/utf8>>, beamai_mustache_rt:to_binary(<<"中文"/utf8>>)),
    ?assertEqual(<<"中文"/utf8>>, beamai_mustache_rt:to_binary([20013, 25991])).

%% Unrenderable values are a template bug and say so instead of being
%% silently formatted with ~p.
to_binary_not_renderable_test() ->
    ?assertError({beamai_mustache, {not_renderable, _}}, beamai_mustache_rt:to_binary(self())),
    ?assertError({beamai_mustache, {not_renderable, _}}, beamai_mustache_rt:to_binary({a, b})).

%%%===================================================================
%%% escape/1
%%%===================================================================

flat(V) -> iolist_to_binary(beamai_mustache_rt:escape(V)).
...
escape_zero_copy_test() ->
    ...
    Got = beamai_mustache_rt:escape(B),
    ?assert(Got =:= B),
...
escape_utf8_passthrough_test() ->
    ...
    Plain = <<"中文🙂"/utf8>>,
    ?assert(erts_debug:same(beamai_mustache_rt:escape(Plain), Plain)).

escape_returns_iodata_test() ->
    ?assert(is_binary(beamai_mustache_rt:escape(<<"plain">>))),
    ?assert(is_list(beamai_mustache_rt:escape(<<"a<b">>))).
...
dot_body() ->
    fun(S, I) -> [I, beamai_mustache_rt:to_binary(
                       beamai_mustache_rt:lookup([?BEAMAI_MUSTACHE_DOT], S))] end.
...
sec(V, Body) -> iolist_to_binary(beamai_mustache_rt:section(V, Body, [#{}], <<>>)).

%% Row 1: the five falsy values run nothing at all. <<>> and false matter
%% most -- generated code has no explicit branch for them.
section_falsy_test() ->
    B = boom_body(),
    ?assertEqual([], beamai_mustache_rt:section(undefined, B, [#{}], <<>>)),
    ?assertEqual([], beamai_mustache_rt:section(false, B, [#{}], <<>>)),
    ?assertEqual([], beamai_mustache_rt:section(null, B, [#{}], <<>>)),
    ?assertEqual([], beamai_mustache_rt:section(<<>>, B, [#{}], <<>>)),
    ?assertEqual([], beamai_mustache_rt:section([], B, [#{}], <<>>)).

%% Row 2: a non-empty list iterates, pushing each element.
section_list_pushes_each_element_test() ->
    Body = fun(S, _I) -> beamai_mustache_rt:to_binary(beamai_mustache_rt:lookup([n], S)) end,
...
section_true_does_not_push_test() ->
    ...
    ?assertEqual(<<"9">>,
                 iolist_to_binary(
                   beamai_mustache_rt:section(true, Body, [#{n => 9}], <<>>))).
...
section_fun2_test() ->
    ...
    ?assertEqual(<<"<inner>">>,
                 iolist_to_binary(
                   beamai_mustache_rt:section(F, Body, [#{k => v}], <<>>))).
...
section_fun1_receives_top_frame_test() ->
    ...
    ?assertEqual(<<"3">>,
                 iolist_to_binary(
                   beamai_mustache_rt:section(F, dot_body(), [#{n => 3}], <<>>))).
...
section_fun1_depth_guard_test() ->
    ...
    ?assertError({beamai_mustache, {lambda_depth_exceeded, _}},
                 beamai_mustache_rt:section(F, dot_body(), [#{}], <<>>)).
...
section_passes_indent_test() ->
    ?assertEqual(<<"  a  b">>,
                 iolist_to_binary(
                   beamai_mustache_rt:section([<<"a">>, <<"b">>], dot_body(),
                                          [#{}], <<"  ">>))).
...
lambda_undefined_test() ->
    ?assertEqual([], beamai_mustache_rt:lambda(undefined, #{})).

lambda_fun1_test() ->
    F = fun(Frame) -> maps:get(who, Frame) end,
    ?assertEqual(<<"world">>,
                 iolist_to_binary(
                   beamai_mustache_rt:lambda(F, #{who => <<"world">>}))).

lambda_fun2_with_value_test() ->
    F = fun(V, Frame) -> [V, maps:get(sep, Frame)] end,
    ?assertEqual(<<"a!">>,
                 iolist_to_binary(
                   beamai_mustache_rt:lambda([F, <<"a">>], #{sep => <<"!">>}))).

lambda_bare_fun2_test() ->
    F = fun(V, _Frame) -> [<<"[">>, V, <<"]">>] end,
    ?assertEqual(<<"[]">>, iolist_to_binary(beamai_mustache_rt:lambda(F, #{}))).

lambda_other_values_render_nothing_test() ->
    ?assertEqual([], beamai_mustache_rt:lambda(<<"str">>, #{})),
    ?assertEqual([], beamai_mustache_rt:lambda(null, #{})),
    ?assertEqual([], beamai_mustache_rt:lambda(42, #{})),
    ?assertEqual([], beamai_mustache_rt:lambda([1, 2], #{})).

%% Lambda output is HTML by design and must not be escaped.
lambda_output_is_not_escaped_test() ->
    F = fun(_Frame) -> <<"<b>&</b>">> end,
    ?assertEqual(<<"<b>&</b>">>,
                 iolist_to_binary(beamai_mustache_rt:lambda(F, #{}))).

lambda_iolist_return_test() ->
    F = fun(_Frame) -> [<<"a">>, [<<"b">>], <<"c">>] end,
    ?assertEqual(<<"abc">>, iolist_to_binary(beamai_mustache_rt:lambda(F, #{}))).
...
lambda_receives_given_frame_test() ->
    F = fun(Frame) -> beamai_mustache_rt:to_binary(Frame) end,
    ?assertEqual(<<"top">>, iolist_to_binary(beamai_mustache_rt:lambda(F, <<"top">>))).
