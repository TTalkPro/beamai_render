%%%-------------------------------------------------------------------
%%% @doc Unit tests for beamai_mustache_scanner.
%%%
%%% The scanner is the only producer of beamai_mustache_token(), so these tests
...
%%% @end
%%%-------------------------------------------------------------------
-module(beamai_mustache_scanner_tests).
...
tokens(Bin) ->
    case beamai_mustache_scanner:scan(Bin) of
        {ok, Ts}       -> Ts;
...
scan_error(Bin, Opts) ->
    Got = try
              beamai_mustache_scanner:scan(Bin, Opts)
          catch
              Cl:Rs:Stack ->
...
%% The scanner does not merge adjacent text; that is beamai_mustache_ast's job.
...
errors_are_never_raised_test() ->
    lists:foreach(
      fun({_Title, Input, _Line, _Reason}) ->
              ?assertMatch({error, {_, _, _}}, beamai_mustache_scanner:scan(Input))
      end, error_cases()).
