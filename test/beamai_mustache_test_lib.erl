...
%%% @end
%%%-------------------------------------------------------------------
-module(beamai_mustache_test_lib).
...
spec_dir() ->
    filename:join(code:lib_dir(beamai_render), "test/spec").
...
%% @doc Convert decoded spec data into an beamai_render context.
%%
%% The spec uses string keys; beamai_render uses atom keys (a deliberate deviation,
...
implemented() ->
    _ = code:ensure_loaded(beamai_mustache),
    erlang:function_exported(beamai_mustache, render_string, 3).

-spec skip_unless_implemented(Config) -> Config | {skip, string()} when Config :: term().
skip_unless_implemented(Config) ->
    case implemented() of
        true  -> Config;
        false -> {skip, "beamai_mustache:render_string/3 not implemented yet "
