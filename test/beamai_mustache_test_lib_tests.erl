...
%%% @end
%%%-------------------------------------------------------------------
-module(beamai_mustache_test_lib_tests).

-include_lib("eunit/include/eunit.hrl").

-import(beamai_mustache_test_lib, [to_ctx/1, name_to_atom/1, name_to_atoms/1]).
...
load_spec_shape_test() ->
    [First | _] = beamai_mustache_test_lib:load_spec(interpolation),
...
load_spec_partials_default_test() ->
    Cases = beamai_mustache_test_lib:load_spec(comments),
    ?assert(lists:all(fun(C) -> maps:get(partials, C) =:= #{} end, Cases)).

load_spec_partials_present_test() ->
    Cases = beamai_mustache_test_lib:load_spec(partials),
    ?assert(lists:any(fun(C) -> maps:get(partials, C) =/= #{} end, Cases)).

required_and_optional_are_disjoint_test() ->
    R = beamai_mustache_test_lib:required_specs(),
    O = beamai_mustache_test_lib:optional_specs(),
