%%%-------------------------------------------------------------------
%%% @doc Tests for beamai_mustache_transform: the three source forms.
%%%
...
%%% @end
%%%-------------------------------------------------------------------
-module(beamai_mustache_transform_tests).

-include_lib("eunit/include/eunit.hrl").

%% A second parse_transform, used to prove beamai_mustache_transform does not
...
tmp_dir() ->
    D = filename:join(code:lib_dir(beamai_render), "tmp_transform_tests"),
...
%% Does any beamai_mustache:inline/2 call survive in these forms?
has_inline(Term) when is_tuple(Term) ->
    case Term of
        {call, _, {remote, _, {atom, _, beamai_mustache}, {atom, _, inline}}, [_, _]} ->
            true;
        _ ->
...
framework_cases() ->
    [{"a module with no mustache constructs comes back untouched",
      fun() ->
              Forms = plain_forms(),
              ?assertEqual(Forms, beamai_mustache_transform:parse_transform(Forms, []))
      end},
     {"running the transform twice changes nothing the second time",
      fun() ->
              Src = "-module(tf_idem).\n"
                    ...
                    "-export([f/1]).\n"
                    "f(N) -> beamai_mustache:inline(<<\"x{{n}}\">>, #{n => N}).\n",
              File = filename:join(tmp_dir(), "tf_idem.erl"),
              ok = file:write_file(File, Src),
              {ok, Forms} = epp:parse_file(File, [], []),
              Once  = beamai_mustache_transform:parse_transform(Forms, []),
              Twice = beamai_mustache_transform:parse_transform(Once, []),
     ...
     {"the module name is found wherever another transform left it",
      fun() ->
              ...
              ?assertEqual(Forms,
                           beamai_mustache_transform:parse_transform(Forms, []))
      end},
     {"another parse_transform running first is fine",
      fun() ->
              Src = "-module(tf_coexist_a).\n"
                    "-compile({parse_transform, beamai_mustache_transform_tests}).\n"
                    "-compile({parse_transform, beamai_mustache_transform}).\n"
                    "-export([f/0]).\n"
                    "f() -> beamai_mustache:inline(<<\"a{{n}}\">>, #{n => 1}).\n",
     ...
     {"another parse_transform running second is fine too",
      fun() ->
              Src = "-module(tf_coexist_b).\n"
                    ...
                    "-compile({parse_transform, beamai_mustache_transform_tests}).\n"
                    "-export([f/0]).\n"
                    "f() -> beamai_mustache:inline(<<\"b{{n}}\">>, #{n => 2}).\n",
     ...
     {"only warnings means {warning, Forms, Ws} and the build continues",
      fun() ->
              Src = "-module(tf_warn_only).\n"
                    "-compile({parse_transform, beamai_mustache_transform}).\n"
                    "-export([f/2]).\n"
                    "f(T, C) -> beamai_mustache:inline(T, C).\n",
...
position_cases() ->
    ...
    [{"every inline call in tf_positions was expanded at compile time",
      fun() ->
              ...
              Out = beamai_mustache_transform:parse_transform(Forms, []),
    ...
    [{"~\"...\" and <<\"...\">> are both literals",
      fun() ->
              Src = "-module(tf_lit).\n"
                    "-compile({parse_transform, beamai_mustache_transform}).\n"
                    "-export([a/0, b/0, c/0]).\n"
                    "a() -> beamai_mustache:inline(~\"s{{v}}\", #{v => 1}).\n"
                    "b() -> beamai_mustache:inline(<<\"d{{v}}\">>, #{v => 2}).\n"
                    "c() -> beamai_mustache:inline(<<\"e\", \"{{v}}\">>, #{v => 3}).\n",
     ...
     {"a macro that expands to a literal binary is expanded too",
      fun() ->
              ...
              ?assertEqual(<<"macro 5">>,
                           beamai_mustache:inline(?TPL, #{v => 5})),
              Src = "-module(tf_macro).\n"
                    "-compile({parse_transform, beamai_mustache_transform}).\n"
                    "-define(T, <<\"macro {{v}}\">>).\n"
                    "-export([f/0]).\n"
                    "f() -> beamai_mustache:inline(?T, #{v => 5}).\n",
     ...
     {"non-literal first arguments are left alone and warn once each",
      fun() ->
              Src = "-module(tf_nonlit).\n"
                    ...
                    "a(T, C) -> beamai_mustache:inline(T, C).\n"
                    "b(T, C) -> beamai_mustache:inline(<<T/binary>>, C).\n"
                    "c(T, C) -> beamai_mustache:inline(list_to_binary(T), C).\n"
                    "d(T, C) -> beamai_mustache:inline(<<T:8>>, C).\n",
     ...
     {"other arities and a local inline/2 are not touched",
      fun() ->
              Src = "-module(tf_notmine).\n"
                    ...
                    "-export([a/0, b/0]).\n"
                    "a() -> inline(<<\"{{v}}\">>, #{v => 1}).\n"
                    "b() -> beamai_mustache:render_string(<<\"{{v}}\">>, #{v => 2}).\n"
                    "inline(_T, _C) -> local.\n",
    ...
    [{"the context expression is evaluated exactly once",
      fun() ->
              Src = "-module(tf_once).\n"
                    ...
                    "f() ->\n"
                    "    put(tf_once_n, 0),\n"
                    "    R = beamai_mustache:inline(<<\"{{a}}{{a}}{{a}}\">>,\n"
                    "                           begin\n"
                    "                               put(tf_once_n,"
                    " get(tf_once_n) + 1),\n"
                    "                               #{a => 1}\n"
     ...
     {"two inline calls in one function do not shadow each other",
      fun() ->
              Src = "-module(tf_two).\n"
                    ...
                    "    S = outer,\n"
                    "    I = also_outer,\n"
                    "    A = beamai_mustache:inline(<<\"a{{v}}\">>, #{v => 1}),\n"
                    "    B = beamai_mustache:inline(<<\"b{{v}}\">>, #{v => 2}),\n"
                    "    {S, I, A, B}.\n",
     ...
     {"a nested section still works when a user variable is called S",
      fun() ->
              Src = "-module(tf_shadow).\n"
                    ...
                    "-export([f/1]).\n"
                    "f(S) ->\n"
                    "    beamai_mustache:inline(<<\"[{{#xs}}({{n}}){{/xs}}]\">>,"
                    " #{xs => S}).\n",
     ...
     {"the same extension tag has no runtime fallback",
      fun() ->
              ...
              ?assertError({beamai_mustache, {error, {_, _, {unknown_marker, $%}}}},
                           beamai_mustache:inline(<<"[{{%b}}x{{/b}}]">>, #{}))
      end}].
...
%% Build a module whose Nth function is `beamai_mustache:inline(<<Tpl>>, Ctx)' and
...
diff_module_forms(Mod, Cases) ->
    ...
    [{attribute, A, file, {atom_to_list(Mod) ++ ".erl", 1}},
     ...
     {attribute, A, export, [{N, 1} || N <- Names]}]
        ++ [{function, A, N, 1,
             [{clause, A, [{var, A, 'Ctx'}], [],
               [{call, A, {remote, A, {atom, A, beamai_mustache}, {atom, A, inline}},
...
diff_test_cases() ->
    ...
    Transformed = beamai_mustache_transform:parse_transform(Forms, []),
    ...
    [{"the expanded module contains no surviving inline call",
     ...
     {"case titles are unique",
      fun() ->
        ...
        ++ [{"interpreted: " ++ Title,
             fun() -> ?assertEqual(Exp, beamai_mustache:inline(Tpl, Ctx)) end}
            || {Title, Tpl, Ctx, Exp} <- Cases]
        ++ [{"byte-identical: " ++ Title,
             fun() ->
                     ?assertEqual(beamai_mustache:inline(Tpl, Ctx),
    ...
    [{"all three attribute shapes are accepted",
     ...
     {"any other shape is rejected with the correct spellings",
      fun() ->
              ...
              Text = lists:flatten(
                       beamai_mustache_transform:format_error(
     ...
     {"but not a template compiled anywhere else",
      fun() ->
              ...
              ?assertMatch({error, {_, _, {unknown_marker, $%}}},
                           beamai_mustache_parser:parse(<<"{{%b}}x{{/b}}">>,
                                                    #{source => <<"t">>}))
      end}].
...
template_cases() ->
    [{"the generated functions render and are exported",
     ...
     {"beamai_html_path:scan/1 sees exactly what the transform collected",
      fun() ->
              ...
              ?assertEqual({ok, [{index, "views/index.mustache"},
                           ...
                           beamai_html_path:scan(File, mustache_template))
      end},
     {"resolve/2 reports every directory it tried",
      fun() ->
              ?assertEqual({error, {not_found, ["a", "b"]}},
                           beamai_html_path:resolve("x.mustache", ["a", "b"])),
              Views = filename:join(fixture_dir(), "views"),
              ?assertEqual({ok, filename:join(Views, "greet.mustache")},
                           beamai_html_path:resolve("greet.mustache",
                                                    ["nope", Views]))
      end},
     {"template_spec/1 derives a name from the basename",
      fun() ->
              ?assertEqual({ok, {index, "a/b/index.mustache"}},
                           beamai_html_path:template_spec("a/b/index.mustache")),
              ?assertEqual({ok, {my_page, "x.mustache"}},
                           beamai_html_path:template_spec({my_page, "x.mustache"})),
              ?assertEqual({ok, {a_b, "a-b.mustache"}},
                           beamai_html_path:template_spec("a-b.mustache")),
              ?assertEqual(error, beamai_html_path:template_spec(42))
      end}].
    ...
    [{"every problem in a module is reported by one compilation",
     ...
     {"format_error covers every reason the transform raises",
      fun() ->
              ...
              [begin
                   Text = lists:flatten(
                            beamai_mustache_transform:format_error(R)),
