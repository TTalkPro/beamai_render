...
build(Name, Base) ->
    ...
    Mod = beamai_mustache_ast:module_name(Name, Base),
    Opts = Base#{module => Mod,
                 source => list_to_binary(Path),
                 stamp  => beamai_mustache_compiler:source_hash(Body, Base),
                 mtime  => 0},
    {ok, Ast} = beamai_mustache_parser:parse(Body, Opts),
    {ok, Forms, _Deps} = beamai_mustache_compiler:forms(Ast, Opts),
...
t_facade() ->
    Ctx = context(),
    ?assertEqual(view_complex:render(Ctx), beamai_mustache:render(view_complex, Ctx)),
    ?assertEqual(view_complex:render_iolist(Ctx),
                 beamai_mustache:render_iolist(view_complex, Ctx)).
...
jinja_parse(Name, Base) ->
    ...
    {ok, Body} = file:read_file(Path),
    Mod = beamai_jinja_ast:module_name(Name, Base),
    Opts = Base#{module => Mod,
                 source => list_to_binary(Path),
                 stamp  => beamai_jinja_compiler:source_hash(Body, Base),
                 mtime  => 0},
    {ok, Nodes} = beamai_jinja_parser:parse(Body, Opts),
    {Mod, Opts, Nodes}.
    ...
    lists:foldl(fun({macro, _, N, _, _}, Acc) -> [N | Acc];
                   (Node, Acc) ->
                        lists:foldl(fun(B, A) -> jinja_macro_names(B) ++ A end,
                                    Acc, beamai_jinja_ast:bodies(Node))
                end, [], Nodes).

jinja_load(Mod, Opts, Nodes) ->
    {ok, Forms, _Deps} = beamai_jinja_compiler:forms(Nodes, Opts),
    {ok, Mod, Bin} = compile:forms(Forms, [return_errors, binary, debug_info]),
    _ = code:purge(Mod),
...
t_jinja_golden() ->
    Expected =
        <<"<!doctype html>\n"
          "<html>\n"
          "  <head><title>beamai_render — Users &amp; guests</title></head>\n"/utf8,
          "  <body>\n"
          "    <h1>Users &amp; guests</h1>\n"
          "    <ul>\n"
