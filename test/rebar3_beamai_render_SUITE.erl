...
%%%
%%% beamai_render and rebar3_beamai_render reach the fixture through _checkouts/. rebar3 has
%%% no `path' resource -- {plugins, [{name, {path, Dir}}]} fails with
%%% fetch_fail -- so _checkouts is the only way to build against a local tree
%%% without a network.
%%% @end
%%%-------------------------------------------------------------------
-module(rebar3_beamai_render_SUITE).

-compile([export_all, nowarn_export_all]).

-include_lib("common_test/include/ct.hrl").
-include_lib("stdlib/include/assert.hrl").
-include("../rebar3_beamai_render/include/rebar3_beamai_render.hrl").
...
up(Dir) ->
    case filelib:is_regular(filename:join([Dir, "rebar3_beamai_render", "rebar.config"]))
        andalso filelib:is_regular(filename:join([Dir, "src",
                                                  "beamai_mustache_compiler.erl"])) of
        true  -> Dir;
...
seed(Priv, Repo) ->
    ...
    copy_app(Repo, filename:join(Seed, "beamai_render"), ["src", "include"]),
    ok = file:write_file(filename:join([Seed, "beamai_render", "rebar.config"]),
                         <<"{erl_opts, [debug_info]}.\n{deps, []}.\n">>),
    copy_app(filename:join(Repo, "rebar3_beamai_render"),
             filename:join(Seed, "rebar3_beamai_render"), ["src", "include"]),
    {ok, _} = file:copy(filename:join([Repo, "rebar3_beamai_render", "rebar.config"]),
                        filename:join([Seed, "rebar3_beamai_render", "rebar.config"])),
    Seed.
...
compile_plugin(Priv, Repo) ->
    ...
    Src = filename:join([Repo, "rebar3_beamai_render", "src"]),
    Inc = filename:join([Repo, "rebar3_beamai_render", "include"]),
...
project(Fixture, Config) ->
    ...
    cp_r(filename:join(Seed, "beamai_render"), filename:join(Checkouts, "beamai_render")),
    cp_r(filename:join(Seed, "rebar3_beamai_render"),
         filename:join(Checkouts, "rebar3_beamai_render")),
    Dir.
...
%% fixture projects, not of beamai_render -- as test modules it cannot find. Renaming
...
%% is not reachable and the plugin has to borrow the project's beamai_render instead.
...
name_mapping(_Config) ->
    Opts = eopts(beamai_mustache_engine, <<"view_">>),
    ?assertEqual(view_index, rebar3_beamai_render_name:module_of(<<"index">>, Opts)),
    ?assertEqual(view_shared_item,
                 rebar3_beamai_render_name:module_of(<<"shared/item">>, Opts)),
    ?assertEqual(view_layout_default,
                 rebar3_beamai_render_name:module_of(<<"layout/default">>, Opts)),
    ?assertEqual(view_a_b, rebar3_beamai_render_name:module_of(<<"a-b">>, Opts)),
    %% shared/item and shared_item really do collide; that is what makes the
    %% collision check necessary rather than paranoid.
    ?assertEqual(rebar3_beamai_render_name:module_of(<<"shared/item">>, Opts),
                 rebar3_beamai_render_name:module_of(<<"shared_item">>, Opts)),
    %% A different prefix moves every module.
    ?assertEqual(tpl_index,
                 rebar3_beamai_render_name:module_of(<<"index">>,
                                              eopts(beamai_mustache_engine, <<"tpl_">>))),
    %% The name is derived from the path exactly as a {{> ...}} would spell it.
    ?assertEqual(<<"shared/item">>,
                 rebar3_beamai_render_name:name_of("/v/shared/item.mustache", "/v",
                                            ".mustache")),
    %% The mapping is the engine's, so a jinja template lands where its own
    %% {% extends %} would look for it.
    JOpts = eopts(beamai_jinja_engine, <<"j2_">>),
    ?assertEqual(j2_index, rebar3_beamai_render_name:module_of(<<"index">>, JOpts)),
    ?assertEqual(j2_layout_base,
                 rebar3_beamai_render_name:module_of(<<"layout/base.j2">>, JOpts)),
    ?assertEqual(<<"layout/base">>,
                 rebar3_beamai_render_name:name_of("/v/layout/base.j2", "/v", ".j2")).
...
illegal_name_detected(_Config) ->
    ...
    ?assertEqual(ok, rebar3_beamai_render_name:validate(view_ok, "views/ok.mustache", Opts)),
    ?assertMatch({error, {_, 0, {illegal_module_name, _, _}}},
                 rebar3_beamai_render_name:validate('view_a+b', "views/a+b.mustache", Opts)),
    %% An empty prefix is legal, but then the name has to carry the lowercase
    %% first letter itself.
    ?assertMatch({error, {_, 0, {illegal_module_name, _, _}}},
                 rebar3_beamai_render_name:validate('Index', "views/Index.mustache",
                                             #mopts{prefix = <<>>})).
...
collision_detection(_Config) ->
    ...
    Errors = rebar3_beamai_render_check:collisions([A, B, C], mopts()),
...
partials_exist_check(_Config) ->
    ...
    %% over a jinja AST.
    ?assertEqual([], rebar3_beamai_render_check:partials_exist(
                       Nodes, Avail, "views/x.j2",
                       (mopts())#mopts{engine = beamai_jinja_engine})).
...
basic_compile(Config) ->
    ...
    %% beamai_render is a library app and the generated code only calls into it.
...
generated_file_shape(Config) ->
    ...
    ?assert(contains(First, "Generated by rebar3_beamai_render")),
    ...
    {ok, #{stamp := Stamp}} =
        rebar3_beamai_render_scan:meta(filename:join([Dir, "_gen",
                                               "view_complex.erl"]),
                                beamai_mustache_engine),
    Body = read(filename:join([Dir, "views", "complex.mustache"])),
    ?assertEqual(beamai_mustache_compiler:source_hash(
